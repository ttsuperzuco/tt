/* 新規予約入力の画面「出し入れの判断」＝パソコン版とスマホ版で共有する唯一の正本。
 *
 * ★まるちゃん決定（2026-08-24）：パソコン版とスマホ版の画面は同じにする。
 *   ところが中身は2か所に別々に手書きされていて、同じ日に3回もズレた
 *   （スマホが先にあきらめて止まる／⑤カウンセリング担当が二重に出る／
 *     「コスモスが埋まってます」が2か所に出る）。
 *   ★どれも「書いてある内容は同じだが順番が逆」で、文字を見比べる点検では見つからなかった。
 *   → **判断だけをこの1本にまとめ、パソコンもスマホも必ずここに聞く。**
 *
 * 【この置き場の決まり】
 *  ・ここは**画面を触らない**（純粋な判断だけ）。「何を出す・何を隠す・どれを選ぶ」を**答えで返す**。
 *    画面の作り（HTMLの組み立て・見た目・名前の付け方）は、パソコン版とスマホ版それぞれのままでよい。
 *  ・**判断を各画面に書き写さない。** 直す時はこのファイルだけを直す。
 *  ・正本＝`AI自動プログラム\共通\画面\新規予約入力_判断.js`。
 *    スマホ版へは `共通\画面\スマホへ配る.py` が同じ中身を写して配る（手で写さない）。
 *    写しがズレていないかは `共通\パソコン版とスマホ版のずれ点検.py` が見る。
 *
 * 呼び方＝どちらの画面からも `NR.〇〇(...)`。
 */
(function (root) {
  'use strict';
  var NR = {};

  /* ── ① 開始時間の文 ───────────────────────────────
     rvdt＝{mm,dd,hh,mi}／weekday＝"水" など。読めていなければ「―」。 */
  NR.startText = function (rvdt, weekday, timeMissing) {
    var d = rvdt;
    if (!d || d.mm == null) return "―";
    // ★時刻が読めなかった用紙は、日付だけ出して「時刻を入れてください」と促す（2026-08-25）。
    if (timeMissing) {
      var wd0 = weekday ? ("（" + weekday + "）") : "";
      return Number(d.mm) + "月" + Number(d.dd) + "日" + wd0 + " ★時刻を入れてください";
    }
    var wd = weekday ? ("（" + weekday + "）") : "";
    var mi = String(d.mi);
    if (mi.length < 2) mi = "0" + mi;
    return Number(d.mm) + "月" + Number(d.dd) + "日" + wd + " " +
           Number(d.hh) + "時" + mi + "分";
  };

  /* ── ② 枠が2つ以上か（＝枠ごとに選ぶ形か） ─────────────
     ★2026-09-11 まるちゃん決定で1つ足した＝**カウンセリングの枠だけが1つ残った時も「枠ごとに選ぶ形」**にする。
       ＝①相談だけの回は施術の枠を作らないので、残るのはカウンセリングの枠1つ。今までの「1つなら外の
       ③所要時間・⑥施術担当・⑦部屋で選ぶ」に落とすと、**使わない施術の部屋を選ばされ、カウンセリング担当を
       選ぶ場所がどこにも出なくなる**（外の⑤はカウンセリングの枠がある時は出さない決まりのため）。 */
  NR.perSlot = function (slots) {
    var S = slots || [];
    if (S.length >= 2) return true;
    return !!(S.length === 1 && S[0] && S[0].kind === "counsel");
  };

  /* ── ②-2 カウンセリングの枠だけか（＝施術をしない回） ─────────
     ★画面が「登録する時に渡す中身」を決めるのに使う（カウンセリング担当は枠の中で選んだ人）。 */
  NR.onlyCounselSlot = function (slots) {
    var S = slots || [];
    return !!(S.length === 1 && S[0] && S[0].kind === "counsel");
  };

  /* ── ③ カウンセリングの枠があるか ─────────────────── */
  NR.hasCounselSlot = function (slots) {
    var S = slots || [];
    for (var i = 0; i < S.length; i++) {
      if (S[i] && S[i].kind === "counsel") return true;
    }
    return false;
  };

  /* ── ④ ③カウンセリングの必要／⑤カウンセリング担当 の出し入れ ──────
     needCounsel＝この予約にカウンセリングが要るか／needcSel＝画面の選択（"no"なら要らない）。
     返り：{ needCSec:③の欄を出すか, counselSec:外の⑤を出すか,
             counselSlotShown:カウンセリングの枠を出すか, rebuildOrder:順番の欄を作り直すか }
     ★外の⑤は「カウンセリングの枠がある＝枠の中で選ぶ」時は**必ず出さない**（二重になるため）。 */
  NR.needCounselView = function (slots, needCounsel, needcSel) {
    var on = !!needCounsel && needcSel !== "no";
    var hasC = NR.hasCounselSlot(slots);
    return {
      needCSec: !!needCounsel,
      counselSec: hasC ? false : on,
      counselSlotShown: on,
      rebuildOrder: hasC
    };
  };

  /* ── ⑤ 「コスモスが埋まってます」の赤い知らせを、外（枠の外）に出すか ──
     ★2026-08-24 まるちゃん指摘の直しどころ。**枠の中に同じ知らせを出す形の時は、外には出さない**
       （出すと1つ目の枠の中と一番下の2か所に同じ物が出る）。
     ★以前スマホ版は、この判断のあとに「埋まり具合でまた出す」を書いていて打ち消していた。
       ここに1本化したので、順番を取り違えようがない。 */
  NR.cosmosWarnOutside = function (av, slots, needCounsel, needcSel) {
    if (NR.hasCounselSlot(slots)) return false;          // 枠の中に出すので外は出さない
    if (!needCounsel || needcSel === "no") return false;  // カウンセリングをやらない回は出さない
    return !!(av && av.cosmos_busy);
  };

  /* ── ⑥ 埋まっている物を選べなくする ────────────────────
     values＝並んでいる選択肢の値の並び／busy＝埋まっている値の並び／cur＝いま選んでいる値。
     返り：{ hidden:[出さない物はtrue], sel:選び直した値(変えないならcurのまま), none:選べる物が0か } */
  NR.hideBusy = function (values, busy, cur) {
    var V = (values || []).map(String);
    var B = (busy || []).map(String);
    var hidden = [], first = null;
    for (var i = 0; i < V.length; i++) {
      var isBusy = B.indexOf(V[i]) >= 0;
      hidden.push(isBusy);
      if (!isBusy && first === null) first = V[i];
    }
    var sel = String(cur == null ? "" : cur);
    var curIdx = V.indexOf(sel);
    // 選んでいた所が埋まったら、空いている先頭に選び直す
    if (curIdx >= 0 && hidden[curIdx] && first !== null) sel = first;
    return { hidden: hidden, sel: sel, none: (first === null) };
  };

  /* ── ⑥-2 「その時間は空いていません」の赤い字を出すか ──────────
     hiddenFlags＝並んでいる選択肢が「出していない」かどうかの並び。全部出していなければ true。 */
  NR.noneLeft = function (hiddenFlags) {
    var H = hiddenFlags || [];
    if (!H.length) return false;
    for (var i = 0; i < H.length; i++) {
      if (!H[i]) return false;
    }
    return true;
  };

  /* ── ⑦ デザイン眉・まつエクの時の見せ方 ──────────────────
     返り：{ pineOnly:担当をパインだけにするか, roomWrapShown:外の⑦部屋を出すか,
             forceStaff:担当を強制する値(無ければ null), clearRoom:部屋を空にするか,
             pickFirstFreeRoom:空いている先頭の部屋を選ぶか } */
  NR.mayuView = function (isMayu, isSat, slots) {
    var ps = NR.perSlot(slots);
    return {
      pineOnly: !!isMayu,
      roomWrapShown: ps ? false : (isMayu ? !!isSat : true),
      forceStaff: isMayu ? "5" : null,
      clearRoom: !!(isMayu && !isSat),
      pickFirstFreeRoom: !!(isMayu && isSat)
    };
  };

  /* ── ⑧ 枠ごとに選ぶ形の時、外の③所要時間・⑥施術担当・⑦部屋を出すか ── */
  NR.outerSectionsShown = function (slots) {
    var ps = NR.perSlot(slots);
    return { dur: !ps, staff: !ps, room: !ps };
  };

  /* ── ⑨ 枠ごとの担当の選択肢 ─────────────────────────
     all＝担当の全部（[値, 表示名, 色] の並び）。
     ★カウンセリングの枠＝トマト(1)・みかん(2)だけ。
     ★施術の枠＝パイン(5)は出さない（デザイン眉・まつエク専門で脱毛はやらないため）。
       ←パソコン版だけパインが出ていた食い違いを、ここでそろえた（2026-08-24）。 */
  NR.slotStaffChoices = function (all, isCounsel) {
    var out = [];
    for (var i = 0; i < (all || []).length; i++) {
      var v = String(all[i][0]);
      if (isCounsel) { if (v === "1" || v === "2") out.push(all[i]); }
      else if (v !== "5") out.push(all[i]);
    }
    return out;
  };

  /* ── ⑩ 枠の開始時刻の積み上げ ────────────────────────
     baseMin＝1つ目が始まる時刻（分）／durs＝枠ごとの分／needcSel＝"no"ならカウンセリングを飛ばす。
     返り：[{ i, startMin, dur, isCounsel, skip }]（skip=trueは作らない枠） */
  NR.slotStarts = function (slots, durs, baseMin, needcSel) {
    var S = slots || [], out = [], pos = Number(baseMin) || 0;
    for (var i = 0; i < S.length; i++) {
      var isC = (S[i] && S[i].kind === "counsel");
      var skip = !!(isC && needcSel === "no");
      var du = Number((durs || [])[i] || 30);
      out.push({ i: i, startMin: pos, dur: du, isCounsel: !!isC, skip: skip });
      if (!skip) pos += du;
    }
    return out;
  };

  /* ── ⑪-0 日付だけの文／時刻だけの文（2026-09-22 まるちゃん）──────────
     登録前の確かめの小窓で使う。**枠は1つずつ別の予約として登録される**ので、
     上には日付だけを出し、開始時刻は枠ごとに出す。 */
  NR.dateText = function (rvdt, weekday) {
    var d = rvdt;
    if (!d || d.mm == null) return "―";
    var wd = weekday ? ("（" + weekday + "）") : "";
    return Number(d.mm) + "月" + Number(d.dd) + "日" + wd;
  };
  NR.hhmm = function (min) {
    var m = Number(min);
    if (!isFinite(m) || m < 0) return "―";
    var h = Math.floor(m / 60), mi = m % 60;
    return h + ":" + (mi < 10 ? "0" + mi : String(mi));
  };

  /* ── ⑪ やる順番の欄 ───────────────────────────────
     返り：{ shown:欄を出すか, rows:[{no, name, canUp}] } */
  NR.orderRows = function (slots) {
    var S = slots || [];
    if (S.length < 2) return { shown: false, rows: [] };
    var rows = [];
    for (var i = 0; i < S.length; i++) {
      rows.push({ no: i + 1, name: (S[i].label || "施術"), canUp: i > 0 });
    }
    return { shown: true, rows: rows };
  };

  /* ── ⑪-1 予約メモに【要確認】が残っていないか ─────────────────
     ★まるちゃん決定（2026-09-04）：**変換した予約メモに【要確認】が残っている間は登録させない**。
       ＝分からないまま登録すると、その中身がそのままお客様への前日お知らせに流れる。
     memo＝白い枠の**今の文**（人が直したあとの文を見る）。
     返り＝要確認になっている物の名前の並び（例 ["メニュー"]）。無ければ空。
     ★名前の取り方（実際に出る書き方を全部見て決めた）：
       「要確認：メニュー」→メニュー ／「要確認：経由 Ig看到」→経由 ／
       「部位を要確認：LINE会話を見る」→部位 ／「経由→【要確認】」→経由（同じ行の頭から取る）。
       どれでも取れない長い文は「予約メモ」とだけ言う（勝手に言葉を作らない）。 */
  NR.pendingChecks = function (memo) {
    var s = String(memo || ""), out = [], re = /【([^】]*)】/g, m;
    while ((m = re.exec(s))) {
      var inner = m[1] || "";
      // ★「確認して修正」も同じ扱い（2026-09-22 まるちゃん決定）＝経由の行はこの言葉に書き換わるので、
      //   ここに足さないと「直っていないのに登録ボタンが押せる」穴が開く。
      // ★2026-09-23 まるちゃん指定でメモの書き方が「【部位入力】」に短くなった。
      //   ここに「入力」を足さないと**直っていないのに登録ボタンが押せる**穴が開く。
      var word = (inner.indexOf("要確認") >= 0) ? "要確認"
               : (inner.indexOf("要記入") >= 0) ? "要記入"
               : (inner.indexOf("確認して修正") >= 0) ? "確認して修正"
               : (inner.indexOf("入力") >= 0) ? "入力" : "";
      if (!word) continue;
      var i = inner.indexOf(word);
      var before = inner.slice(0, i).replace(/[をがの・\s　]+$/, "").split(/[・、]/).pop();
      var after = inner.slice(i + word.length).replace(/^[：:\s　]+/, "").split(/[\s　＝=、。]/)[0];
      var lab = "";
      if (before && before.length <= 14) lab = before;
      else if (after && after.length <= 14) lab = after;
      if (!lab) {                       // 同じ行の【より前（例「経由→」）から取る
        var head = s.slice(0, m.index).split("\n").pop() || "";
        head = head.replace(/[◉●・\s　]/g, "").replace(/[→:：]+$/, "");
        if (head && head.length <= 14) lab = head;
      }
      if (!lab) lab = "予約メモ";
      if (out.indexOf(lab) < 0) out.push(lab);
    }
    return out;
  };

  /* ── ⑪-1b 直す所の知らせ（赤い行・経由の書き換え） ─────────────
     ★まるちゃん決定（2026-09-22）：白い枠の下に出していた一行は、みんな読み落とす
       （まるちゃん「これメッセージみんなよみおとす」）→ 赤い行で枠の上に出す形にした。
     決まり（2つ）：
       ①**小窓（メッセージの箱）は出さない**（2026-09-22 まるちゃん「ここの前に出るメッセージBOXは
         ださない。ここに修正があってわかるから」）。いったん小窓を出す形にしたが、赤い行が
         同じ画面に出るので二度手間だった。popup は必ず空を返す。
       ②**経由の行だけ**「経由→【確認して修正】」に書き換える。
         経由いがい（メニュー・部位など）は書き換えない＝お客様が書いた元の言葉が消えると
         直しようが無くなるため（2026-09-22 まるちゃん決定）。
       ③白い枠の**すぐ上**に、赤い字で「◯◯ を修正してください」を出す（2026-09-22 まるちゃん指定の文）。
         （白い枠は色を付けられない普通の文字の箱なので、枠の中ではなく外に出す＝まるちゃん①あ）
     memo＝読み取った文（または人が直したあとの文）。
     返り：{ items:[名前…], memo:書き換え後の文, popup:必ず空, red:赤い行(無ければ空) } */
  NR.fixNotice = function (memo) {
    var s = String(memo || "");
    // 経由の行だけ書き換える（「経由→【…要確認/要記入…】」→「経由→【確認して修正】」）
    s = s.replace(/^([^\S\n]*[◉●・]?[^\S\n]*経由[^\S\n]*[→:：][^\S\n]*)【[^】]*(?:要確認|要記入)[^】]*】/gm,
                  "$1【確認して修正】");
    // ★小窓（メッセージの箱）は出さない（2026-09-22 まるちゃん「ここの前に出るメッセージBOXはださない。
    //   ここに修正があってわかるから」）＝赤い行だけで足りるため、popup は必ず空にする。
    var items = NR.pendingChecks(s), red = "";
    // ★2026-09-23 まるちゃん指定：どこを見て直すかを添える（メモの中は短くしたため）。
    if (items.length) red = items.join("・") + " を修正してください（LINEのやりとりを参照）";
    return { items: items, memo: s, popup: "", red: red };
  };

  /* ── ⑪-1c 1つずつ画面を進める（2026-09-22 まるちゃん決定・聞く物は同じ画面に混ぜない） ──
     まるちゃん「画面をいっしょにすんなって。カウンセリング聞く画面を先にやってから、
     予約メモの確認する画面だよ」。
     画面の並び＝
       貼り付け →（プロセルはどれ？）→（カウンセリングのみ？）→ 開始時間 → 予約メモの確認
       → 担当・部屋など（登録）。
     かっこの中は**聞く必要がある時だけ**出す（要らない人は飛ばす）。
     ★2026-09-22 まるちゃん「カウンセリグ有無のあとにこれ」＝**開始時間はカウンセリングのすぐ後**
       （予約メモの確認より前）。
     ★聞く画面を予約メモより**前**に置く理由＝押すと予約メモの文がその場で作り直されるので、
       後ろに置くと人が手で直した文が消えるため。
     hasProcell／hasCouns＝読み取った時に聞く必要があったか。返り＝出す画面の名前の並び。
     ★2026-09-22 まるちゃん決定（この日2回目の分割）＝**担当・部屋のあとを、さらに3つに分ける**。
       「性別選択は次の画面にする。で その次が、タイトル 全部画面わける」
       ＝ … → 担当・部屋など → **性別・国籍** → **タイトル（ここで登録）**。
       まるちゃん指定＝①カウンセリングの必要・やる順番・所要時間・担当・部屋は同じ画面のまま
       （空きを見ながら一緒に選ぶため）②国籍は性別と同じ画面。
     needGender＝性別・国籍のどちらかを人に選ばせる必要があるか（両方とも貼り付けた文から
       読めていれば、その画面は空っぽになるので飛ばす）。
     ★★2026-09-23 まるちゃん決定（この日3回目の分割）＝**1画面1つの用事にする**。
       「開始時間の画面の次に ③カウンセリングの必要。次の画面に移ってから やる順番。そのあと
        所要時間（『所要時間（分）』だけじゃ分からないから『〇〇の所要時間（分）』）。その後 予約メモ。
        そのあと 〇〇 担当・部屋。その画面は、カウンセリング・施術が二つあるならそれぞれ。
        つまり『〇月〇日〇時 カウンセリング／担当／部屋』、次の画面が『〇月〇日〇時 ハイドラ』、
        次『〇月〇日〇時 学生応援』みたいな感じ」。
       ＝並び＝… 開始時間 → カウンセリングの必要 → やる順番 → 所要時間 → 予約メモ
         → 施術ごとの担当・部屋（1つにつき1画面）→ 性別・国籍 → タイトル。
     ★★2026-09-23 まるちゃん決定（同じ日の直し）＝**やる順番・所要時間を「予約メモの後ろ」へ動かす**。
       まるちゃん「カウンセリングだけじゃなくて、施術の時間もいれないとだめだし、この時点では
       施術内容きまってない（予約メモがまだかいてない）から、予約メモのあとの時間入力じゃない？」
       ＝**どんな施術をやるかは予約メモで決まる**。メモを確かめる前に時間や順番を聞いても、
       そこに並ぶのは貼った用紙から読めた分だけで、人がメモに書き足した施術が抜ける
       （実際に「当日施術」が空欄の用紙で、カウンセリングの時間しか出なかった）。
       ＝新しい並び＝… 開始時間 → カウンセリングの必要 → **予約メモ** → やる順番
         → 〇〇の所要時間 → 施術ごとの担当・部屋 → 性別・国籍 → タイトル。
       ★これに合わせて、画面は**予約メモを確かめた後に、そのメモから枠を作り直す**
         （事務所パソコンの「メモ→枠」に聞き直す）。
     opts＝{ needcShown:カウンセリングの必要を聞くか, orderShown:やる順番の欄を出すか,
             slotCount:枠ごとに分ける時の枠の数（0＝今までの1画面） }。 */
  NR.stepList = function (hasProcell, hasCouns, needGender, opts) {
    var o = opts || {};
    var a = ["paste"];
    if (hasProcell) a.push("procell");
    if (hasCouns) a.push("couns");
    a.push("time");
    if (o.needcShown) a.push("needc");
    a.push("memo");
    if (o.orderShown) a.push("order");
    a.push("dur");
    var n = Number(o.slotCount || 0);
    if (n > 0) {
      for (var i = 0; i < n; i++) a.push("slot" + i);
    } else {
      a.push("rest");
    }
    if (needGender !== false) a.push("gender");
    a.push("title");
    return a;
  };

  /* どの画面を出すか（画面の名前で聞く）。slot＝枠ごとの画面の番号（-1＝ちがう）。 */
  NR.stepView = function (name) {
    var n = String(name || "paste");
    var sl = /^slot(\d+)$/.exec(n);
    return { name: n, paste: n === "paste", procell: n === "procell", couns: n === "couns",
             memo: n === "memo", time: n === "time", rest: n === "rest",
             gender: n === "gender", title: n === "title",
             needc: n === "needc", order: n === "order", dur: n === "dur",
             slot: sl ? Number(sl[1]) : -1 };
  };

  /* ── 枠ごとの画面の見出し（2026-09-23 まるちゃん）───────────────
     例＝「12月1日（火）19:00～　ハイドラ」。dateText＝NR.dateText の答え。 */
  NR.slotHeadText = function (dateText, hhmm, label) {
    var d = String(dateText || "");
    var t = hhmm ? (String(hhmm) + "～") : "";
    var s = String(label || "施術");
    return (d ? (d + "　") : "") + (t ? (t + "　") : "") + s;
  };

  /* 所要時間の見出し（例＝「ハイドラの所要時間（分）」）。 */
  NR.durHeadText = function (label) {
    return String(label || "施術") + "の所要時間（分）";
  };

  /* ── 性別・国籍の画面が要るか（2026-09-22）───────────────
     貼り付けた文から**両方とも読めていれば要らない**（画面が空っぽになるため飛ばす）。 */
  NR.needGenderStep = function (gender, tw) {
    return !(String(gender || "") && String(tw || ""));
  };

  /* ── 性別・国籍の画面で次へ進ませてよいか（2026-09-22）──────
     どちらも選ばれていないと、タイトルが作れない（タイトルに入る情報のため）。 */
  NR.genderOk = function (gender, tw) {
    var miss = [];
    if (!String(gender || "")) miss.push("性別");
    if (!String(tw || "")) miss.push("国籍");
    return { ok: miss.length === 0,
             msg: miss.length ? (miss.join("と") + "を選んでください。") : "" };
  };

  /* ── ⑪-1e プロセルが顔か頭皮か決まっていないか ───────────────
     ★お店の書き方ルール（2026-08-21）でメモは「プロ肌／プロ頭／プロ」になるので、その言葉でも見分ける。 */
  NR.procellAsk = function (memo) {
    var s = String(memo || "");
    var has = /procell/i.test(s) || /プロセル/.test(s) || /プロ肌|プロ頭|プロ(?!グラム)/.test(s);
    var face = /顔プロセル/.test(s) || /プロ肌/.test(s);
    var scalp = /頭皮プロセル/.test(s) || /プロ頭/.test(s);
    return { ask: !!(has && !face && !scalp) };
  };
  NR.procellOk = function (memo) {
    var a = NR.procellAsk(memo);
    return { ok: !a.ask, msg: a.ask ? "どのプロセルかを選んでください。" : "" };
  };

  /* ── ⑪-1d 次の画面へ進ませてよいか ─────────────────────
     ★直す所（【要確認】【確認して修正】）が残っている間は進ませない（2026-09-22 まるちゃん）
       ＝登録ボタンと同じ考え。分からないまま先へ行かせない。
     ★時刻が読めなかった用紙は、時刻を入れるまで「この時間でOK」を押させない。 */
  NR.memoOk = function (memo) {
    var pend = NR.pendingChecks(memo);
    return { ok: pend.length === 0,
             msg: pend.length ? (pend.join("・") + "を直してから「予約メモはこれでOK」を押してください。") : "" };
  };
  /* ★時間を変えたら、空いている部屋・担当を取り直し終わるまで次へ行かせない
     （2026-09-22 まるちゃん「時間かえたら、あいてる部屋や担当も読み直しだろ」）。
     取り直しは事務所パソコンに聞きに行くので数秒かかる。待たずに進めると、
     ふさがっている部屋・担当のまま登録の画面へ行けてしまう。 */
  NR.timeOk = function (timeMissing, checking) {
    if (timeMissing) {
      return { ok: false, msg: "開始時間が読み取れませんでした。上の「修正」から時刻を入れてください。" };
    }
    // ★★2026-09-23 まるちゃん決定：**ここでは空きの取り直しを待たせない。裏で続ける。**
    //   まるちゃん「ここで担当者と空き部屋検索しないで。裏でやって。担当と部屋選択のときに
    //   でてればいいから。そのときまでに処理がおわってなかったら、BOXがでて、
    //   けんさくちゅうちょっとまってがでればいい」。
    //   ＝待つ場所を「開始時間の画面」から「担当・部屋を選ぶ画面」へ移した
    //   （2026-09-22の「調べ終わるまで押させない」はここでは外す）。取り直しが終わる前に
    //   担当・部屋を選ぶ画面へ着いたら、その画面で小窓を出して待ってもらう。
    return { ok: true, msg: "", checking: !!checking };
  };

  /* ── 相談の回（①②③）を選んだか（2026-09-23 まるちゃん決定）───────────
     ★選ばないと枠が決まらず、施術の枠が無いまま先へ進んでしまう
       （実際に「当日施術」が空欄の用紙で、所要時間も担当・部屋もカウンセリングの分しか
        出ないまま進めた）。①②③のどれかを押すまで「これでOK」を押させない。 */
  NR.counselOk = function (kind) {
    var k = String(kind || "");
    var ok = (k === "1" || k === "2" || k === "3");
    return { ok: ok, msg: ok ? "" : "①②③のどれかを選んでください。" };
  };

  /* ── 担当・部屋を選ぶ画面に着いた時、まだ空きを調べているか（2026-09-23 まるちゃん）─────
     true なら「空いている担当・部屋を調べています。少しお待ちください。」の小窓を出す。 */
  NR.availWaitView = function (avPending, timeChecking) {
    return { shown: !!(avPending || timeChecking),
             msg: "空いている担当・部屋を調べています。少しお待ちください。" };
  };

  /* ── カウンセリングの部屋がふさがっている時の知らせ（2026-09-23 まるちゃん）───────
     まるちゃん「欄内の文字の説明のいみがわかりにくい」。直した所＝
       ①**結論を先に**「この時間はカウンセリングができません」
       ②次の行で**理由**（誰が使っているか）
       ③最後に**次にやること**（画面に本当にあるボタンの名前で書く）。
     ★「上の『開始時間』」をやめて「← 戻る」にした＝開始時間は別の画面に移ったので、
       いま見ている画面を探しても無く、スタッフが迷う（2026-09-22 に別の文では直していたが、
       ここだけ古い言い方が残っていた）。
     ★2026-09-23（同じ日の2回目）まるちゃん「14:00〜14:30 🍅預約M430тw新規　ふさがっています。
       っていみがわからない」＝**時刻と予約の題名を文の途中に挟んだので、日本語として読めなかった**。
       しかも、その文字が何なのか（＝その部屋に入っている別の予約）を書いていなかった。
       → 「別の予約が入っています」と言葉で言い、**入っている予約は次の行に分けて出す**。
     hhmm＝その枠が始まる時刻の文字（例「14:00」）／
     busyText＝その部屋に入っている別の予約（例「14:00〜14:30 🍅預約M430тw新規」）。 */
  NR.cosmosBusyText = function (hhmm, busyText) {
    var t = String(hhmm || "");
    var w = String(busyText || "").trim();
    var L = [];
    L.push("⚠ " + (t ? (t + "〜は") : "この時間は") + "カウンセリングができません。");
    L.push("カウンセリングの部屋（コスモス）に、別の予約が入っています。");
    if (w) L.push("入っている予約＝" + w);
    L.push("「← 戻る」で開始時間を変えるか、やる順番を変えてください。");
    return L.join("\n");
  };

  /* ── 施術ごとの担当・部屋の画面で「これでOK」を押せるか・下に出す一言（2026-09-23 まるちゃん）──
     ★まるちゃん「担当をえらんでください　のもじいる？」＝**部屋がふさがっていて選ぶボタンが
       1つも出ていない時に「選んでください」と出ていた**（選びようがないのに選べと言われる）。
       その時は上の赤い知らせが次にやることを書いているので、一言は出さない。
     need＝まだ選べていない物の名前の並び（例 ["担当","部屋"]）／waiting＝空きを調べている最中か／
     locked＝その枠は部屋がふさがっていて選ぶボタンを出していないか。 */
  NR.slotGo = function (need, waiting, locked) {
    var n = need || [];
    if (waiting) {
      return { disabled: true, msg: "空いている担当・部屋を調べています。少しお待ちください。" };
    }
    if (locked) { return { disabled: true, msg: "" }; }
    if (n.length) { return { disabled: true, msg: n.join("と") + "を選んでください。" }; }
    return { disabled: false, msg: "" };
  };

  /* ── 予約メモを確かめた後、そのメモから枠を作り直している間（2026-09-23 まるちゃん）─────
     ★どんな施術をやるかは予約メモで決まるので、メモの「これでOK」を押したら、
       そのメモを事務所パソコンへ渡して枠（施術の並び）を作り直す。その間に出す小窓。 */
  NR.slotsWaitView = function (pending) {
    return { shown: !!pending,
             msg: "予約メモから施術の枠を作っています。少しお待ちください。" };
  };

  /* ── ⑪-2 登録してよいか（決まっていない枠があれば押させない） ──────
     ★まるちゃん決定（2026-08-24）：「カウンセリングと脱毛のどちらか、あるいは複数のサービスの
       どれかが**確定できない状態**のときは、登録ボタンが押せないようにする」。
       ＝登録の処理は空き具合をいっさい見ないので、押せてしまうと**同じ部屋に二重で予約が入る**。
     rows＝画面が集めた枠ごとの状態の並び：
       { name:枠の名前, locked:その時間は使えない(部屋がふさがっている等),
         staffOk:担当が決まっているか, roomNeeded:部屋が要るか, roomOk:部屋が決まっているか }
     checking＝**まだ空き具合の答えを待っている**（true の間は押させない）。
       ★これが要る理由＝枠ごとの空きは事務所パソコンに聞きに行くので数秒あとに届く。
         その間ボタンが押せると、ふさがっているのに登録できてしまう隙ができる（実測で起きた）。
     memo＝白い枠の今の文（【要確認】が残っている間は押させない・2026-09-04 まるちゃん決定）。
     返り：{ ok:登録してよいか, ng:[{name, why}], msg:画面に出す1行 } */
  NR.canRegister = function (rows, checking, timeMissing, memo, need) {
    // ★★2026-09-11 まるちゃん決定：**性別か国籍が決まっていない間は登録させない。**
    //   ＝タイトルが「預約?新規」のように？のまま登録されてしまう（国籍の🇹🇼も付かない）。
    //   分からない方だけを画面に出して選んでもらう（両方分からなければ両方出す）。
    var _n = need || {};
    if (_n.gender || _n.tw) {
      var _w = [];
      if (_n.gender) _w.push("性別");
      if (_n.tw) _w.push("国籍");
      var _ng = [];
      for (var _i = 0; _i < _w.length; _i++) _ng.push({ name: _w[_i], why: "決まっていません" });
      return { ok: false, ng: _ng, msg: _w.join("・") + "を選択してください" };
    }
    // ★予約メモに【要確認】が残っている間は登録させない（2026-09-04 まるちゃん決定）。
    //   ＝分からないまま登録すると、その中身がそのままお客様への前日お知らせに流れる。
    var pend = NR.pendingChecks(memo);
    if (pend.length) {
      var ngp = [];
      for (var p = 0; p < pend.length; p++) ngp.push({ name: pend[p], why: "要確認のままです" });
      return { ok: false, ng: ngp,
               // ★言い方（2026-09-04 まるちゃん）＝「正しい内容に」とは言わない。
               //   機械は**【要確認】の字が残っているかどうかだけ**を見ていて、正しいかどうかは
               //   見ていない（正しいかを決めるのは直す人）。実態どおりの言葉にする。
               //   ★2026-09-22：経由の行は【確認して修正】に書き換わるので「要確認になっています」
               //     では字が合わない。どちらの印にも当てはまる言い方にそろえた。
               msg: pend.join("・") + "が直っていません。予約メモを書き直してから" +
                    "「この内容で登録する」を押してください。" };
    }
    // ★時刻が読めなかった用紙（昔は時刻の欄が無かった）＝人が「開始時間」を入れるまで登録させない
    //   （2026-08-25 まるちゃん決定）。
    if (timeMissing) {
      return { ok: false, ng: [{ name: "", why: "開始時間が入っていません" }],
               msg: "開始時間が読み取れませんでした。上の「修正」から時刻を入れてください。" };
    }
    if (checking) {
      return { ok: false, ng: [{ name: "", why: "確かめ中" }],
               msg: "空き具合を確かめています。少しお待ちください。" };
    }
    var R = rows || [], ng = [];
    for (var i = 0; i < R.length; i++) {
      var r = R[i] || {}, nm = r.name || ("枠" + (i + 1));
      if (r.locked) { ng.push({ name: nm, why: "その時間は部屋がふさがっています" }); continue; }
      if (!r.staffOk) { ng.push({ name: nm, why: "担当が決まっていません" }); continue; }
      if (r.roomNeeded && !r.roomOk) { ng.push({ name: nm, why: "部屋が決まっていません" }); }
    }
    var msg = "";
    if (ng.length) {
      var parts = [];
      for (var k = 0; k < ng.length; k++) parts.push(ng[k].name + "：" + ng[k].why);
      // ★2026-09-22 まるちゃん決定：「上の」をやめて「← 戻る」にした。開始時間は別の画面へ
      //   移ったので、いま見ている画面を探しても「開始時間」が無く、スタッフが迷うため。
      msg = "この内容では登録できません（" + parts.join("／") +
            "）。「← 戻る」で開始時間を変えるか、やる順番を変えてください。";
    }
    return { ok: (ng.length === 0), ng: ng, msg: msg };
  };

  /* ── ⑪-3 タイトルの欄を出すか ────────────────────────
     ★まるちゃん決定（2026-08-25）：**担当か部屋のどちらかが選べない時は、タイトルを出さない。**
       理由＝担当も部屋も決まらないと**中身が確定しない**ので、出しても意味がない
       （まるちゃん「確定ができないんだから、何を内容を」）。登録ボタンは今までどおり押せないまま。
     can＝NR.canRegister の答え。返り：{ shown:タイトルの欄を出すか } */
  NR.titleView = function (can) {
    return { shown: !!(can && can.ok) };
  };

  /* ── ⑫ 順番の入れ替え（枠と、選んでいる中身を一緒に動かす） ─────
     picks＝[{dur,staff,room}...]（枠と同じ並び）。返り：{ slots, picks } */
  NR.moveUp = function (slots, picks, i) {
    var S = (slots || []).slice(), P = (picks || []).slice();
    if (i <= 0 || i >= S.length) return { slots: S, picks: P };
    var t = S[i - 1]; S[i - 1] = S[i]; S[i] = t;
    var u = P[i - 1]; P[i - 1] = P[i]; P[i] = u;
    for (var k = 0; k < S.length; k++) {
      if (!S[k] || !P[k]) continue;
      S[k].dur = P[k].dur; S[k].staff = P[k].staff; S[k].room = P[k].room;
    }
    return { slots: S, picks: P };
  };

  /* ── ⑬ 相談（カウンセリング）の回かどうかの欄 ──────────────
     ★まるちゃん決定（2026-08-25）：脱毛のたびに必ずカウンセリングはやる。分かれるのは**お客様の気持ち**＝
       ①相談だけ ②相談してから施術するか決める ③ふつうの施術。これはお客様の文からしか読めないので
       事務所パソコンでAIが読む。**AIは決めない＝初期値を入れるだけで、スタッフが押して直せる。**
     kind＝AIの答え（"1"／"2"／"3"／"要確認"）。hint＝お客様の文かLINEに相談の話が出ていたか。
     返り：{ shown:欄を出すか, sel:最初に選ばれている物（"1"/"2"/"3"／要確認なら""）,
             ask:人に選ばせるか（要確認）, label:見出しの文 } */
  NR.counselingView = function (kind, hint, hasHair) {
    var k = String(kind || "3");
    var shown = !!hint || (k === "1" || k === "2" || k === "要確認");
    // ★相談（カウンセリング）の①②は脱毛の話だけ。脱毛が無い予約では欄そのものを出さない
    //   （2026-09-11 まるちゃん「カウンセリングだけなのは脱毛だけ。あとは影響させないで」）。
    //   ★渡されなかった時は今までどおり出す（古い呼び方でも動く）。
    if (hasHair === false) shown = false;
    return {
      shown: shown,
      sel: (k === "要確認" ? "" : k),
      ask: (k === "要確認"),
      // ★2026-09-11 まるちゃん：画面では「相談」の言葉を一切使わない（分かりにくいため）。
      label: (k === "要確認"
              ? "カウンセリングのみか、施術もやるかが分かりませんでした。選んでください"
              : "カウンセリングのみの予約ですか？")
    };
  };

  /* ── ⑬-1b AIがそう判断した理由の1行 ─────────────────────
     ★2026-09-11 まるちゃん：画面に出す文で「相談」の言葉を使わない（AIの書いた理由も言い換える）。
     返り：出す文（無ければ空）。 */
  /* ★2026-09-22 まるちゃん「この文はいらん」＝**画面に理由の1行は出さない**ことにした。
     どの画面からも呼ばない（空を返すだけ）。中身は経緯として残す。 */
  NR.counselingWhy = function (why) {
    return "";
  };

  /* ── ⑬-2 選んだ①②③に合うメモを返す ───────────────────
     ★画面は「どう書き直すか」を**自分で決めない**。事務所パソコンが①②③の3通りを先に作って渡すので、
       画面はその中から選ぶだけ（判断を画面に書き写さない＝二重管理を作らない）。
     memos＝{"1":…,"2":…,"3":…}／now＝今の欄の文（3通りが無い時はそのまま返す）。 */
  NR.counselingMemo = function (memos, kind, now) {
    var m = memos && memos[String(kind)];
    var out = (typeof m === "string" && m) ? m : (now || "");
    // ★①②③を押すと読み取ったままの文に戻るので、経由の書き換えもやり直す（2026-09-22 まるちゃん）。
    //   やり直さないと、赤い知らせは「経由→【確認して修正】」なのに白い枠だけ【要確認：…】に戻る。
    return NR.fixNotice(out).memo;
  };

  /* ── ⑬-3 ①②③を選んだ時、枠をどう作るか ─────────────────
     ★まるちゃん決定（2026-09-11）：**②相談してから決める＝相談の枠のあとに施術の枠も先に押さえる。**
       過去の実データでも、はじめてのお客様4人のうち3人が相談＋施術の2枠で取っていた
       （野沢実穂様2024-08-27／加藤喬様2024-12-14／中村健示郎様2026-09-12。1枠だけは新内藍様2025-06-12）。
       当日やることになった時に部屋が無い、を防ぐため。
     ★①相談だけ＝**施術の枠は作らない**（メモも「※施術はなし」なので、部屋を押さえたままにしない）。
       ①②のメモは施術の行が1行にまとまるので、**残す施術の枠も1つだけ**にそろえる。
     ★③ふつうの施術＝読み取ったままの枠（何も変えない）。
     base＝読み取った時の枠の並び（元の並びを画面が覚えておき、押し直すたびにここから作り直す）。
     返り＝その選択で使う枠の並び。 */
  NR.counselingSlots = function (base, kind) {
    var S = (base || []).slice(), k = String(kind || "3");
    if (k !== "1" && k !== "2") return S;
    var out = [], hairUsed = false, hair = null, i, s;
    for (i = 0; i < S.length; i++) {
      s = S[i];
      if (!s) continue;
      if (s.kind === "counsel") { out.push(s); continue; }
      // ★印の付く施術（ハイドラ・プロセル・マツヤニ・パリジェンヌ等）は相談と関係ないので必ず残す
      //   （2026-09-11 まるちゃん「カウンセリングだけなのは脱毛だけ。あとは影響させないで」）。
      if (s.mark) { out.push(s); continue; }
      if (!hair) hair = s;                       // 印の無い枠＝脱毛
      if (k === "2" && !hairUsed) { out.push(s); hairUsed = true; }
    }
    // 相談の枠が無い方（＝既にご利用のお客様）で、残る枠が1つも無い時は脱毛の枠を1つ残す
    //   ＝相談もその部屋で行うため（予約が1件も作られないのを防ぐ）。
    if (!out.length) out = hair ? [hair] : S;
    return out;
  };

  root.NR = NR;
})(typeof window !== "undefined" ? window : (typeof globalThis !== "undefined" ? globalThis : this));
