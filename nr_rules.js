...
(function (root) {
  ...
  /* ── ⑪-1b 直す所の知らせ（小窓の文・赤い行・経由の書き換え） ─────────
     ★まるちゃん決定（2026-09-22）：**貼り付けて読み取った直後、直す所があれば小窓を出す。**
       白い枠の下に出していた一行は、みんな読み落とすため（まるちゃん「これメッセージみんなよみおとす」）。
     決まり（3つ）：
       ①小窓の文＝「【◯◯】を読み取ることができません。LINEのメッセージのやりとりで【◯◯】を
         確認し、予約メモを直接修正してください。」（◯◯＝直す所の名前）。
     ...
     返り：{ items:[名前…], memo:書き換え後の文, popup:小窓の文, red:赤い行(無ければ空) } */
  NR.fixNotice = function (memo) {
    var s = String(memo || "");
    // 経由の行だけ書き換える（「経由→【…要確認/要記入…】」→「経由→【確認して修正】」）
    s = s.replace(/^([^\S\n]*[◉●・]?[^\S\n]*経由[^\S\n]*[→:：][^\S\n]*)【[^】]*(?:要確認|要記入)[^】]*】/gm,
                  "$1【確認して修正】");
    var items = NR.pendingChecks(s), red = "", popup = "";
    if (items.length) {
      red = items.join("・") + " を修正してください";
      var n = "【" + items.join("】【") + "】";
      popup = n + "を読み取ることができません。\n" +
              "LINEのメッセージのやりとりで" + n + "を確認し、予約メモを直接修正してください。";
    }
