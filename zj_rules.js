...
(function (root) {
  ...
  /* ── ② どのボタンを出すか ─────────────────────────────
     ★まるちゃん決定（2026-08-25）：**誰が開いても2つとも出す**
       （スタッフのスマホにも・🍅トマトのスマホにも「未送信の分だけ作成」を出す）。
       事務所パソコンの画面は前から2つとも出していたので、これで**パソコンとスマホが同じ**になる。
     ※2026-08-24 は「全員分を作成の1つだけ・未送信は開発の時だけ」だったが、2026-08-25 に逆転。
     引数 isDev は残してあるが、いまは答えを変えない（呼ぶ側を直さなくてよいように）。
     返り：{ all:出すか, unsent:出すか, hint:下に出す一言 } */
  ZJ.optionsShown = function (isDev) {
    return {
      all: true,
      unsent: true,
      hint: "日付を選んで、どちらかのボタンを押すとお知らせが作成されます。"
    };
  };
