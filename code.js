...
function renderNewReservationPage_(base, staff, dev) {
  ...
  var script = '<script>(function(){' +
    ...
    'if(_titleKeep&&NR.titleView(r).shown){var _k=_titleKeep;_titleKeep=null;fillTitles(_k.t,_k.d);return;}' +
    // ★2026-09-23 まるちゃん「（タイトルが）でない」＝作れなかった時に理由を出さず、
    //   「作っています」のまま永久に止まっていた。理由を出して、何が起きたか分かるようにする。
    'if(_titleErr){btn.disabled=true;if(ngbox){ngbox.textContent="タイトルが作れませんでした。"+_titleErr+"\n「← 戻る」で予約メモや担当・部屋を見直してください。";ngbox.style.display="";}}' +
    ...
    // ★★2026-09-23 まるちゃん「（タイトルが）でない」への直し。
    //   前は、答えが返ってきた時にまだ出せない場面（空きを調べている最中など）だと**答えを捨てて**
    //   いた。作り直しの合図も出ないので「タイトルを作っています」のまま**永久に止まった**。
    //   → 出せない時は答えを**とっておき**、出せるようになった時（nrGoCheck）に出す。
    ...
    'var tpolls=0;function pollTitles(id,myReq,cb){tpolls++;if(tpolls>LIMITS.tries("preview_new_titles",400)){if(cb)cb();return;}jsonp({action:"status",key:KEY,id:id},function(r){if(!r||!r.ok){if(cb)cb();return;}if(r.status==="pending"||r.status==="running"||r.status==="queued"||r.status===""){setTimeout(function(){pollTitles(id,myReq,cb);},400);return;}if(r.status!=="done"){if(cb)cb();return;}var d={};try{d=JSON.parse(r.result||"{}");}catch(e){}if(!d||!d.ok){_titleErr=(d&&d.error)?String(d.error):"タイトルが作れませんでした。";titleStale=false;var _we=document.getElementById("nrTitleWrap");if(_we)_we.innerHTML=\'<div class=\\"nrnote2\\">タイトルが作れませんでした</div>\';nrGoCheck();if(cb)cb();return;}if(myReq!==titleReq||titleEdited){if(cb)cb();return;}fillTitles(d.titles||[],d.disps||[]);if(cb)cb();});}' +
...
function renderExistingPage_(base, staff, dev, mode) {
  ...
  var script = '<script>(function(){' +
    ...
    'document.getElementById("exToDate").addEventListener("click",function(){if(!digits){szPopup_("お客様番号を入れてください");return;}if(ISCHANGE){loadPicks();}else{SGX.goMonth();}});' +
    ...
    'function rvUpdateNowMemoDo(){var el=document.getElementById("exrvnowmemo");if(!el||!rvctx)return;var decisions=[];for(var i=0;i<rvitems.length;i++){var it=rvitems[i];decisions.push({line_no:it.line_no,do:it.do,count:it.count,finish:it.finish});}var fields={new_memo:rvctx.new_memo||"",decisions:decisions,new_items:rvNewItems.slice(),date:rvctx._ymd,time:rvctx._tm,booking_services:rvBookingTexts(),paid:exPaid()};jsonp({action:"submit",key:KEY,op:"existing_apply_memo",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify(fields)},function(r){if(!r||!r.ok||!r.id){return;}var tries=0;(function pll(){tries++;if(tries>60){var el3=document.getElementById("exrvnowmemo");if(el3&&rvMemoOv==null)el3.value="（時間がかかっています。もう一度お試しください）";return;}jsonp({action:"status",key:KEY,id:r.id},function(st){if(!st||!st.ok)return;if(st.status==="pending"||st.status==="running"||st.status==="queued"||st.status===""){setTimeout(pll,500);return;}if(st.status!=="done")return;var d={};try{d=JSON.parse(st.result||"{}");}catch(e){}var el2=document.getElementById("exrvnowmemo");if(el2&&rvMemoOv==null)el2.value=(d&&d.ok)?(d.memo||""):"（メモを作れませんでした）";exPayRemovedNote(el2);});})();});}' +
    ...
    'function hideSteps(){var ids=["exNum","exPick","exMenu","exDate","exTime","exEdit","exMemo","exStaffPick","exRoomPick","exResv","exrvMenu","sgbackbar","sgMonth","sgDay","sgFree","sgTime"];for(var i=0;i<ids.length;i++){var el=document.getElementById(ids[i]);if(el)el.style.display="none";}setExTop();}' +
    ...
    'function exPrefetch(date){if(PREF&&PREF.date===date&&!PREF.err)return;var me={date:date,ctx:null,err:"",wait:null};PREF=me;' +
      'jsonp({action:"submit",key:KEY,op:"existing_build_ctx",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify({number:disp(),date:date,time:""})},function(r){' +
        ...
        'tryData();});}' +
    'function exPrefDone(me){if(me.ctx&&me.ctx.found){SGX.setAhead(me.ctx.paid_ahead||{});SGX.setAsk(!!me.ctx.ask);}var w=me.wait;me.wait=null;if(w)w();}' +
    ...
    'function exGoMemo(st){if(!PREF||PREF.date!==st.date)exPrefetch(st.date);var me=PREF;' +
      ...
      /* 払ったかを聞く方なのに、まだ答えていない（読み終わる前に押した）＝予約時間設定に戻って聞く。 */
      'if(me.ctx.ask&&st.paid===null){SGX.setAsk(true);SGX.toTime();return;}' +
    ...
    'var SGX=(function(){' +
      'if(!document.getElementById("sgMonth"))return {goMonth:function(){},toTime:function(){},setAsk:function(){},setAhead:function(){},state:function(){return {};}};' +
      ...
      'function sgOnDate(d){PAID=null;PAIDAUTO="";exPrefetch(d);ASK=!!(PREF&&PREF.ctx&&PREF.ctx.found&&PREF.ctx.ask);' +
  ...
  return '<style>' + HOMECSS_ + css + (isChange ? '' : sgCss_() + AKFCSS_) + '</style>' +
    ...
    '<div class="ex">' + numSec + pickSec + menuSec + dateSec + timeSec + editSec + memoSec + staffPickSec + roomPickSec + resvSec + resvMenuSec + '</div></div>' + script;
}
