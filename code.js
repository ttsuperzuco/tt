...
function renderNewReservationPage_(base, staff, dev) {
  ...
  var script = '<script>(function(){' +
    ...
    'var _pab=document.querySelectorAll("[data-procell]");for(var _i=0;_i<_pab.length;_i++){_pab[_i].addEventListener("click",function(){var nm=this.getAttribute("data-procell");if(nm==="顔プロセルPro"||nm==="顔プロセルMD"){nm=nm+" "+(window.__procellWord||"トライアル");}prevEl.value=prevEl.value.replace(/(?:顔|頭皮)?プロセル|procell/gi,nm);prevEl.style.height="auto";prevEl.style.height=(prevEl.scrollHeight+6)+"px";var _pa=document.getElementById("nrProcellAsk");if(_pa)_pa.style.display="none";});}' +
...
function renderExistingPage_(base, staff, dev, mode) {
  ...
  var resvSec =
    '<div id="exResv" style="display:none">' +
      ...
      '<div class="exsec">今回のメモ（登録される実際のメモ）</div><div class="rvprevmemo" id="exrvnowmemo">（作成中…）</div>' +
  ...
  var script = '<script>(function(){' +
    ...
    'function showRvMenu(q){hideSteps();document.getElementById("exrvMenu").style.display="";document.getElementById("exrvMenuSearch").value=q||"";drawRvMenuList(q||"");window.scrollTo(0,0);}' +
    'function drawRvMenuList(q){q=(q||"").toLowerCase();var ms=(rvctx.menus||[]);var h="";for(var i=0;i<ms.length;i++){var nm=ms[i].name;if(q&&nm.toLowerCase().indexOf(q)<0)continue;h+="<button type=\\"button\\" class=\\"pickrow\\" data-mn=\\""+esc(nm)+"\\" style=\\"text-align:left\\">"+esc(nm)+(ms[i].cat?"<span style=\\"color:#94a3b8;font-size:14px;margin-left:8px\\">"+esc(ms[i].cat)+"</span>":"")+"</button>";}document.getElementById("exrvMenuList").innerHTML=h||"<div class=\\"exhint\\">見つかりません</div>";}' +
    'document.getElementById("exrvMenuSearch").addEventListener("input",function(){drawRvMenuList(this.value);});' +
    'document.getElementById("exrvMenuList").addEventListener("click",function(e){var b=e.target.closest("[data-mn]");if(!b)return;rvAddItem(b.getAttribute("data-mn"));hideSteps();document.getElementById("exResv").style.display="";window.scrollTo(0,document.body.scrollHeight);});' +
    'document.getElementById("exrvMenuBack").addEventListener("click",function(){hideSteps();document.getElementById("exResv").style.display="";window.scrollTo(0,0);});' +
    'function rvAutoMarkOn(mk){var tm=(rvctx.title_marks||[]);var words=[];for(var i=0;i<tm.length;i++){if(tm[i][0]===mk){words=tm[i][1]||[];break;}}var doing=rvDoingTexts().join(" ");for(var j=0;j<words.length;j++){if(doing.indexOf(words[j])>=0)return true;}return false;}' +
    ...
    'function rvUpdateNowMemoDo(){var el=document.getElementById("exrvnowmemo");if(!el||!rvctx)return;var decisions=[];for(var i=0;i<rvitems.length;i++){var it=rvitems[i];decisions.push({line_no:it.line_no,do:it.do,count:it.count,finish:it.finish});}var fields={new_memo:rvctx.new_memo||"",decisions:decisions,new_items:rvNewItems.slice()};jsonp({action:"submit",key:KEY,op:"existing_apply_memo",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify(fields)},function(r){if(!r||!r.ok||!r.id){return;}var tries=0;(function pll(){tries++;if(tries>20)return;jsonp({action:"status",key:KEY,id:r.id},function(st){if(!st||!st.ok)return;if(st.status==="pending"||st.status==="running"||st.status==="queued"||st.status===""){setTimeout(pll,500);return;}if(st.status!=="done")return;var d={};try{d=JSON.parse(st.result||"{}");}catch(e){}var el2=document.getElementById("exrvnowmemo");if(el2)el2.textContent=(d&&d.ok)?(d.memo||""):"（メモを作れませんでした）";});})();});}' +
    ...
    'function doRegister(){var decisions=[];for(var i=0;i<rvitems.length;i++){var it=rvitems[i];decisions.push({line_no:it.line_no,do:it.do,count:it.count,finish:it.finish});}var doing=rvDoingTexts();var roomKey=(rvsel.room==="STAR/福/🇫🇷")?"STAR":rvsel.room;var fields={number:disp(),date:rvctx._ymd,time:rvctx._tm,dur:parseInt(rvsel.dur,10)||30,room_key:roomKey,prev_title:rvctx.prev_title||"",staff_emoji:SEMO_[rvsel.staff]||"",doing_texts:doing,suffix:rvSuffix(),marks:rvEffMarks(),new_memo:rvctx.new_memo||"",decisions:decisions,new_items:rvNewItems.slice(),title_override:(rvTitleOv!=null?rvTitleOv:"")};exOvShow_(szBusyHtml_("予約を登録中です"),"#2C7A99");jsonp({action:"submit",key:KEY,op:"existing_create",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify(fields)},function(r){if(!r||!r.ok||!r.id){exOvHide_();szPopup_("エラーが発生しました。通信に失敗しました。もう一度お試しください。");return;}pollRegister(r.id,roomKey);});}' +
