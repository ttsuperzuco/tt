...
'try{var sg=sessionStorage.getItem("sz_home_group");if(sg&&document.getElementById("group-"+sg)){menu.style.display="none";var g2=document.querySelectorAll(".group");for(var k=0;k<g2.length;k++)g2[k].style.display=(g2[k].id==="group-"+sg)?"":"none";}}catch(e){}' +
'})();</script>';
...
function renderBroadcastPage_(base, staff, dev) {
  ...
  var css =
    ...
    '.bccampkinds{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:0 0 6px;}' +
    '.bccampkind{padding:16px 8px;font-size:16px;font-weight:800;border:0;border-radius:12px;' +
    'background:#2563EB;color:#fff;line-height:1.35;word-break:break-all;box-shadow:0 3px 10px rgba(0,0,0,.22);cursor:pointer;}' +
    '.bccampkind.on{background:#16A34A;box-shadow:0 0 0 3px #fff inset;}' +
    '.bccampkind.on::before{content:"✓ ";}' +
    '.bccamptab{margin:12px 0 14px;}' +
    '.bcpick.bccampon{border-color:#16A34A;box-shadow:0 0 0 2px #16A34A inset;}' +
    '.bcpick.bccampon span{color:#7dd3a0;}' +
    // ★欄（上から送る順）＝1つずつ枠で囲み、見出しの右に ▲▼・消す（2026-09-17）
    '.bcslot{background:#0B1220;border:1px solid #26324A;border-radius:12px;padding:10px 10px 12px;margin:0 0 10px;}' +
    '.bcslothead{display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin:0 0 10px;}';
  var script =
  '<script>(function(){' +
  'var EXEC="' + EXEC + '",KEY="' + KEY + '";' +
  'var idn=(window.__SZ_WHO_!==undefined)?{who:window.__SZ_WHO_||"",role:window.__SZ_ROLE_||"",device:window.__SZ_DEVICE_||""}:{who:"",role:"",device:""};' +
  ...
  'else if(page==="c"){drawCampKind();}else if(page==="ce"){drawCampEdit();}' +
  ...
  'function drawKind(){' +
  // ★2026-09-17 まるちゃん依頼で「キャンペーン・商品の紹介」を足した。
  'var h=\'<button type="button" class="bckind" id="bckind1">予約可能枠案内</button>\'+' +
  '\'<button type="button" class="bckind" id="bckind2">キャンペーン・商品の紹介</button>\'+' +
  ...
  'document.getElementById("bckind2").onclick=function(){MOVED=true;campEnter();};' +
  ...
  'if(d.ok&&CMODE){campDone();step=0;page="l";mode="";LPOSTS=null;draw();}' +
  'else if(d.ok){DATA=TPL.map(function(){return {parts:[]};});' +
  ...
  // ★キャンペーン・商品の紹介（2026-09-17）。中身は下の bcCampCode_ にふつうの書き方で書いてある。
  bcCampSrc_() +
  // ── 立ち上がり ────────────────────────────────────────
  ...
  'if(CMODE&&page!=="l")return campBack();' +
...
// ★★2026-09-17 まるちゃん「既存の予約も、日付入力から予約メモの手前まで施術後の予約とまったく同じに」。
//   施術後の予約の3〜6枚目（月→日→その日の空き状況→予約時間設定）を部品にした。施術後の予約と既存の予約の
//   両方がこの同じ部品を使う＝見た目も動きも最初から同じ・片方だけ直してずれることが無い。
...
var SG_STEPS_JS_ =
    'var DAYSTAFF=[];' +
    /* ★2026-09-25 まるちゃん決定：**日付を選ぶ前に施術時間を決める**ので、その長さが入らない空きは
       押せなくする（薄く出す）＝「そうすれば空き時間が正確になる」。0＝長さが決まっていない（今までどおり）。
       施術後の予約は0のままなので、あちらの動きは変わらない。 */
    'var NEEDMIN=0;' +
    ...
    'function drawCal(){' +
      ...
      'for(var k=0;k<ds.length;k++){if(ds[k].getAttribute("data-d")){ds[k].onclick=function(){' +
        ...
        /* ★既存の予約は、日にちを押した瞬間に前回の予約を読み始める（2026-09-17）。施術後の予約には無い。 */
        'if(typeof sgOnDate==="function")sgOnDate(PICKDATE);' +
        'goFree();};}}}' +
    ...
    'function drawFree(){' +
      ...
      'DAYROOMS=day.rooms_free||[];DAYSTAFF=day.staff||[];' +
      ...
      'for(var f=0;f<fs.length;f++){' +
        ...
        'fs[f].style.cursor="pointer";fs[f].onclick=function(){' +
        'if(!tapOK())return;' +
    ...
    'function freeRooms(a,b){' +
      ...
      'return out;}' +
    /* ★2026-09-17 まるちゃん決定：写す元の予約メモに、選んだ日の『支払い済み／未払い』がもう書いてあれば、
       「次回の費用は支払済みですか？」をその答えで始める（先に払っていただいた分を「いいえ」で消さないため）。
       答えの中身は事務所パソコンの共通の1本（treatment_memo.paid_ahead）が出す＝ここでは日付を引くだけ。
       人が押し直したら人の答えが勝つ。日にちを選び直したら、自動で入れた答えは外す。 */
    'function aheadPick(){' +
      'var k=PICKDATE?String(PICKDATE).slice(5).replace("-","/"):"";' +
      'if(PAIDAUTO&&PAIDAUTO!==k){PAID=null;PAIDAUTO="";}' +
      'if(!ASK||PAID!==null||!k||!AHEAD)return;' +
      'if(AHEAD[k]==="1"||AHEAD[k]==="0"){PAID=(AHEAD[k]==="1")?1:0;PAIDAUTO=k;}}' +
    'function drawTime(){' +
      'aheadPick();' +
    ...
    'function sgBindSteps(){var ps=document.getElementsByClassName("sgtpm");' +
      'for(var i=0;i<ps.length;i++){ps[i].onclick=function(){' +
        ...
        'drawTime();};}' +
      '$("sgpaid1").onclick=function(){if(!tapOK())return;PAID=1;PAIDAUTO="";drawTime();};' +
      '$("sgpaid0").onclick=function(){if(!tapOK())return;PAID=0;PAIDAUTO="";drawTime();};' +
    '}' +
    '';
...
function renderAfterTreatmentPage_(base, staff, dev, who) {
  ...
  var script =
    ...
    'var PAID=null;' +
    /* 先の日付の答え（{"10/01":"1"}）と、自動で入れた答えの日付。消した『払った／未払い』の行。 */
    'var AHEAD={},PAIDAUTO="",PAYREMOVED=[];' +
    ...
    'function drawMemoDone(){var el=$("sgmdone");' +
      ...
      /* ★2026-09-17：消した前回までの『払った／未払い』の行も出す（本当にまだ頂いていない時に気づけるように）。 */
      'for(var r=0;r<PAYREMOVED.length;r++){h+="<div>お支払い状況から <b>"+esc(PAYREMOVED[r])+"</b> を消しました</div>";}' +
...
function renderNewReservationPage_(base, staff, dev) {
  ...
  var script = '<script>(function(){' +
    ...
    // ★2026-09-23 まるちゃん：1画面1つの用事にした分（カウンセリングの必要／やる順番／所要時間／
    //   施術ごとの担当・部屋）。枠ごとの画面は、同じ入れ物の中の該当する1つだけを出す。
    'b=document.getElementById("nrStepNeedc");if(b)b.style.display=v.needc?"":"none";' +
    'b=document.getElementById("nrStepOrder");if(b)b.style.display=v.order?"":"none";' +
    'b=document.getElementById("nrStepDur");if(b)b.style.display=v.dur?"":"none";' +
    'b=document.getElementById("nrStepSlot");if(b)b.style.display=(v.slot>=0)?"":"none";' +
    'if(v.slot>=0)nrShowOneSlot(v.slot);' +
    'if(v.rest)nrShowRestHead();' +
    ...
    'if(v.rest||v.slot>=0)nrAvailWaitSync();else nrAvailWaitHide();' +
    ...
    'function nrShowRestHead(){var hd=document.getElementById("nrRestHead");if(!hd)return;' +
    'var S=window.__nrSlots||[],lb=(S[0]&&S[0].label)||"施術";' +
    'hd.textContent=NR.slotHeadText(NR.dateText(window.__rvdt,window.__nrWd),' +
    'NR.hhmm(Number((window.__rvdt||{}).hh)*60+Number((window.__rvdt||{}).mi)),lb);}' +
    // ★並びの材料＝カウンセリングの必要を聞くか／やる順番の欄を出すか／枠ごとに分ける数。
    //   「必要なし」を選ぶと枠が1つ減るので、そのたびに並びを作り直す（画面の数が変わる）。
    'function nrStepOpts(){var S=window.__nrSlots||[];' +
    'var nv=NR.needCounselView(S,window.__nrNeedCounsel,sel.needc);' +
    'var n=0;if(NR.perSlot(S)){var durs=[];for(var q=0;q<S.length;q++)durs.push(sel["dur#"+q]);' +
    'var ps=NR.slotStarts(S,durs,0,sel.needc);' +
    'for(var i=0;i<S.length;i++){if(!ps[i].skip)n++;}}' +
    'return {needcShown:!!nv.needCSec, orderShown:!!NR.orderRows(S).shown, slotCount:n};}' +
    'function nrRebuildSteps(){var L=window.__nrSteps||[],now=L[window.__nrStepI||0];' +
    'window.__nrSteps=NR.stepList(window.__nrHasProcell,window.__nrHasCouns,' +
    'NR.needGenderStep((window.__nrReadGender||""),(window.__nrReadTw||"")),nrStepOpts());' +
    'var j=window.__nrSteps.indexOf(now);nrShowStep(j>=0?j:(window.__nrStepI||0));}' +
    'window.__nrRebuildSteps=nrRebuildSteps;' +
    ...
    // ★2026-09-23 まるちゃん：施術ごとの担当・部屋の画面＝その枠が決まるまで次へ進ませない。
    'var _si=window.__nrSlotNow,_sb=document.getElementById("nrSlotOk"),_sng=document.getElementById("nrSlotNg");' +
    'if(_sb&&typeof _si==="number"&&_si>=0){var _S2=window.__nrSlots||[],_isC2=(_S2[_si]&&_S2[_si].kind==="counsel");' +
    ...
    'if(tv.shown&&!titleEdited&&!_titleErr&&!document.querySelector(".nrTitleIn"))refreshTitles();' +
    // ★タイトルを作り直している間は登録させない（古い性別のまま登録されるのを防ぐ）。
    'if(_titleKeep&&tv.shown){var _k=_titleKeep;_titleKeep=null;fillTitles(_k.t,_k.d);return;}' +
    // ★作れなかった時は理由を出す（前は「作っています」のまま永久に止まっていた）。
    'if(_titleErr){btn.disabled=true;if(ngbox){ngbox.textContent="タイトルが作れませんでした（"+_titleErr+"）。「← 戻る」で予約メモや担当・部屋を見直してください。";ngbox.style.display="";}}' +
    'else if(titleStale){btn.disabled=true;if(ngbox){ngbox.textContent="タイトルを作っています。少しお待ちください。";ngbox.style.display="";}}}' +
    ...
    'if(g==="needc"){nrApplyNeedC();nrScheduleAvail();if(window.__nrRebuildSteps)window.__nrRebuildSteps();}' +
    'else if(g==="dur"||g.indexOf("dur#")===0){nrScheduleAvail();}nrGoCheck();});' +
    ...
    //   所要時間は1画面にまとめて並べ（各見出しに施術の名前）、担当・部屋は施術ごとに1画面。
    'var h="",hd="";for(var i=0;i<slots.length;i++){var s=slots[i];' +
    ...
    // 所要時間の画面（まとめて並べる）
    'hd+="<div class=\\"nrdurrow\\" data-di=\\""+i+"\\"><div class=\\"nrsec\\">"+esc(NR.durHeadText(s.label||(isC?"カウンセリング":"施術")))+"</div>"' +
    '+"<div class=\\"nrpills nrdur\\">"+dp+"</div></div>";' +
    // 担当・部屋の画面（施術ごと）
    'h+="<div class=\\"nrslot\\" data-si=\\""+i+"\\">"+(isC?("<div class=\\"nrcoswarn\\" id=\\"cosWarn"+i+"\\" style=\\"display:none\\"></div>"):"")' +
    ...
    'var dl=document.getElementById("nrDurList");if(dl)dl.innerHTML=hd;' +
    'buildOrderUI();}' +
    ...
    'window.__nrReadGender=d.gender||"";window.__nrReadTw=d.tw||"";' +
    ...
    'window.__nrSteps=NR.stepList(window.__nrHasProcell,window.__nrHasCouns,NR.needGenderStep(d.gender,d.tw),nrStepOpts());nrShowStep(1);' +
    ...
    'var titleEdited=false,titleReq=0,_titleTimer=null,titleStale=false,_titleKeep=null,_titleErr="";' +
    ...
    // ★★2026-09-23 まるちゃん「（タイトルが）でない」への直し。前は、答えが返ってきた時に
    //   まだ出せない場面（空きを調べている最中など）だと**答えを捨てて**いたので、
    //   「作っています」のまま止まることがあった。→ とっておいて、出せるようになったら出す。
    'if(!NR.titleView(window.__nrCanReg).shown){_titleKeep={t:titles,d:disps};sec.style.display="none";titleStale=false;nrGoCheck();return;}' +
    '_titleKeep=null;' +
    ...
    'var tpolls=0;function pollTitles(id,myReq,cb){tpolls++;if(tpolls>LIMITS.tries("preview_new_titles",400)){if(cb)cb();return;}jsonp({action:"status",key:KEY,id:id},function(r){if(!r||!r.ok){if(cb)cb();return;}if(r.status==="pending"||r.status==="running"||r.status==="queued"||r.status===""){setTimeout(function(){pollTitles(id,myReq,cb);},400);return;}if(r.status!=="done"){if(cb)cb();return;}var d={};try{d=JSON.parse(r.result||"{}");}catch(e){}if(!d||!d.ok){_titleErr=(d&&d.error)?String(d.error):"わけが分かりません";titleStale=false;nrGoCheck();if(cb)cb();return;}if(myReq!==titleReq||titleEdited){if(cb)cb();return;}fillTitles(d.titles||[],d.disps||[]);if(cb)cb();});}' +
    ...
    'function markTitleStale(){titleStale=true;_titleKeep=null;_titleErr="";var w=document.getElementById("nrTitleWrap");' +
  ...
  return '<style>' + HOMECSS_ + css + '</style>' +
    ...
    '<div class="nr">' +
      ...
      '</div>' +
      // ── 画面4＝担当・部屋などの残り ──
      // ── 画面＝カウンセリングの必要（2026-09-23 まるちゃん「1画面1つの用事」） ──
      '<div id="nrStepNeedc" style="display:none">' +
        '<div id="secNeedC"><div class="nrsec">カウンセリングの必要</div><div class="nrpills">' +
        ...
        '<button type="button" class="nrgo" id="nrNeedcOk">これでOK</button>' +
      '</div>' +
      // ── 画面＝やる順番 ──
      '<div id="nrStepOrder" style="display:none">' +
        '<div id="secOrder"><div class="nrsec">やる順番（上から順にやります）</div>' +
        '<div class="nrnote2">順番を変えたい物の「↑ 上へ」を押してください</div><div id="nrOrderList"></div></div>' +
        '<button type="button" class="nrgo" id="nrOrderOk">これでOK</button>' +
      '</div>' +
      // ── 画面＝所要時間（見出しに施術の名前を入れる＝まるちゃん「〇〇の所要時間（分）」） ──
      '<div id="nrStepDur" style="display:none">' +
        '<div id="secDurWrap"><div class="nrsec" id="nrDurHead1">所要時間（分）</div><div class="nrpills nrdur">' + durPills + '</div></div>' +
        '<div id="nrDurList"></div>' +
        '<button type="button" class="nrgo" id="nrDurOk">これでOK</button>' +
      '</div>' +
      // ── 画面＝施術ごとの担当・部屋（1つにつき1画面・見出しに日付と開始時刻） ──
      '<div id="nrStepSlot" style="display:none">' +
        '<div class="nrsec" id="nrSlotHead"></div>' +
        '<div id="nrSlotWrap"></div>' +
        '<div id="nrSlotNg" class="nrwarn" style="display:none"></div>' +
        '<button type="button" class="nrgo" id="nrSlotOk">これでOK</button>' +
      '</div>' +
...
function renderExistingPage_(base, staff, dev, mode) {
  ...
  var numSec =
    '<div id="exNum">' +
      ...
      '<textarea class="exbox" id="exdisp" rows="2" placeholder="ここにお客様番号を貼り付けるか、下のボタンでお客様番号を入力する"></textarea>' +
  ...
  var resvSec =
    '<div id="exResv" style="display:none">' +
      ...
      '<div id="exrvsingle">' +
        '<div id="exrvstaffwrap"><div class="exsec">施術担当</div><div class="mvng" id="exrvstaffnone" style="display:none">その時間に空いている担当がいません</div><div class="rvbusy" id="exrvstaffbusy" style="display:none">前回と同じ担当はこの時間は空いていません。空いている人から選んでください。</div><div class="expills" id="exrvstaff">' + rvStaffP + '</div></div>' +
      ...
      '<div class="exsec">TTに登録される予約メモです。間違いがないか確認してください（修正可能）</div><textarea class="rvprevmemo rvmemoedit" id="exrvnowmemo">（作成中…）</textarea>' +
      '<button class="exgo" id="exResvGo">この内容でTimeTreeに登録する</button>' +
      '</div>' +
    '</div>';
  // ★★2026-09-25 まるちゃん決定の新しい流れ（既存の予約）＝
  //   番号 →「今回も〇〇をやりますか？」→「〇〇以外にやるメニューは？」→（2つ以上なら）担当・部屋を分けるか
  //   →（3つ以上で分けるなら）どれとどれが同じか → 施術時間 → 月 → 日 → 空き状況 → 時間・部屋 → メモ確認。
  //   **施術を先に決める**ので施術時間が先に出せて、空き状況もその長さで正しく出せる。
  var askSec =
    '<div id="exAsk" style="display:none">' +
      '<div class="ubar"><a class="uhome" id="exbackAsk" href="javascript:void(0)">← 戻る</a></div>' +
      '<div class="exwho1" id="exaskwho"></div>' +
      '<div id="exaskprev"></div>' +
      '<div class="exmh" id="exaskq">今回も同じ施術をやりますか？</div>' +
      '<button type="button" class="exgo" id="exaskyes">はい</button>' +
      '<button type="button" class="exgo" id="exaskno" style="background:#475569;margin-top:8px">いいえ</button>' +
    '</div>';
  var pickItemsSec =
    '<div id="exPickItems" style="display:none">' +
      '<div class="ubar"><a class="uhome" id="exbackPickItems" href="javascript:void(0)">← 戻る</a></div>' +
      '<div class="exmh">今回やる施術を選んでください</div>' +
      '<div id="expkitems"></div>' +
      '<div id="expknew"></div>' +
      '<button type="button" class="exgo" id="expkAdd" style="background:#2563eb;margin:8px 0 4px">＋ メニューから足す</button>' +
      '<button type="button" class="exgo" id="expkGo">これで決定→</button>' +
    '</div>';
  var addSec =
    '<div id="exAdd" style="display:none">' +
  ...
  var splitSec =
    ...
    '</div>';
  var groupSec =
    '<div id="exGroup" style="display:none">' +
      '<div class="ubar"><a class="uhome" id="exbackGroup" href="javascript:void(0)">← 戻る</a></div>' +
      '<div class="exmh">同じ担当＆部屋にする施術には、同じ番号を選んでください</div>' +
      '<div id="exgrplist"></div>' +
      '<button type="button" class="exgo" id="exgrpGo">これで決定→</button>' +
    '</div>';
  var durSec =
  ...
  var script = '<script>(function(){' +
    ...
    'document.getElementById("exToDate").addEventListener("click",function(){if(!digits){szPopup_("お客様番号を入れてください");return;}' +
      'if(ISCHANGE){loadPicks();return;}' +
      /* ★2026-09-25：番号のつぎは日付ではなく「今回も〇〇をやりますか？」（施術を先に決める）。 */
      'PREF=null;EXSEL=null;EXFLOW=null;SGX.setNeed(0);exWaitCtx(exShowAsk);});' +
    ...
    'function pollBuildCtx(id,ymd,tm){var nm="exctx_"+String(id).toLowerCase().replace(/[^a-z0-9_]/g,"")+".json",n=0,fin=false;function got(ctx){if(fin)return;fin=true;exOvHide_();if(!ctx||!ctx.found){szPopup_("「"+disp()+"」の前回の予約が見つかりません。");return;}rvctx=ctx;rvctx._ymd=ymd;rvctx._tm=tm;showResvEdit();}function fail(m){if(fin)return;fin=true;exOvHide_();szPopup_(m);}function tryData(){if(fin)return;n++;jsonp({action:"data",name:nm},function(ctx){if(fin)return;if(ctx&&ctx.ok){got(ctx);return;}if(n%3===0){jsonp({action:"status",key:KEY,id:id},function(r){if(fin)return;if(!r||!r.ok){setTimeout(tryData,700);return;}if(r.status==="pending"||r.status==="running"||r.status==="queued"||r.status===""||r.status==="done"){setTimeout(tryData,r.status==="done"?0:700);return;}fail(esc(r.result)||"エラーが発生しました。りゅうさんにお伝えください。");});return;}setTimeout(tryData,700);});}tryData();}' +
    ...
    'document.getElementById("exrvMenuList").addEventListener("click",function(e){var cb=e.target.closest("[data-rvcat]");if(cb){_rvcat=cb.getAttribute("data-rvcat");_rvsub="";drawRvMenuList("");return;}var sb=e.target.closest("[data-rvsub]");if(sb){_rvsub=sb.getAttribute("data-rvsub");drawRvMenuList("");return;}var b=e.target.closest("[data-mn]");if(!b)return;rvAddItem(b.getAttribute("data-mn"));_rvcat="";_rvsub="";' +
      /* ★2026-09-25：足し終わったら「来た画面」へ戻る（新しい流れの途中から開くこともあるため）。 */
      'if(EXMENUBACK==="exPickItems"){exShowPick();return;}if(EXMENUBACK==="exAdd"){exShowAdd();return;}' +
      'hideSteps();document.getElementById("exResv").style.display="";window.scrollTo(0,document.body.scrollHeight);});' +
    'document.getElementById("exrvMenuBack").addEventListener("click",function(){var sq=document.getElementById("exrvMenuSearch");if(sq&&sq.value){sq.value="";drawRvMenuList("");return;}if(_rvsub){_rvsub="";drawRvMenuList("");return;}if(_rvcat){_rvcat="";drawRvMenuList("");return;}' +
      'if(EXMENUBACK==="exPickItems"){exShowPick();return;}if(EXMENUBACK==="exAdd"){exShowAdd();return;}' +
      'hideSteps();document.getElementById("exResv").style.display="";window.scrollTo(0,0);});' +
    ...
    'function exGoBusy_(on){var b=document.getElementById("exResvGo");if(!b)return;' +
      /* 確認画面が出ている時だけ効かせる（手前の画面で印や枠を変えた時は何もしない） */
      'var B=document.getElementById("exrvB");' +
      'if(on){if(!B||B.style.display==="none")return;' +
      'if(!b.getAttribute("data-txt"))b.setAttribute("data-txt",b.textContent);' +
      'b.disabled=true;b.style.opacity="0.45";b.style.cursor="default";b.textContent="メモを作成中です…";}' +
      'else{b.disabled=false;b.style.opacity="";b.style.cursor="";' +
      'var t=b.getAttribute("data-txt");if(t){b.textContent=t;b.removeAttribute("data-txt");}}}' +
    'var _nowMemoT=null;function rvUpdateNowMemo(){exGoBusy_(true);if(_nowMemoT)clearTimeout(_nowMemoT);_nowMemoT=setTimeout(rvUpdateNowMemoDo,700);}' +
    /* ★★2026-09-25 まるちゃん決定の新しい流れ（ここから）──────────────────────────
       番号 →「今回も〇〇をやりますか？」→「〇〇以外にやるメニューは？」→（2つ以上なら）分けますか
       →（3つ以上で分けるなら）どれとどれが同じか → 施術時間 → 月 → 日 → 空き状況 → 時間・部屋 → メモ確認。 */
    'var EXFLOW=null,EXMENUBACK="exResv";' +
    'var EXDURS=[15,20,30,40,45,50,60,70,80,90,120,150];' +
    'function exEl(i){return document.getElementById(i);}' +
    'function exOpen(id){hideSteps();var e=exEl(id);if(e)e.style.display="";window.scrollTo(0,0);}' +
    /* 材料（前回の予約・施術・施術時間の目安）を、日付を決める前に取る。 */
    'function exWaitCtx(cb){if(!PREF||PREF.err)exPrefetch("");var me=PREF;' +
      ...
      'if(!me.ctx){exOvShow_(szBusyHtml_("前回の予約を読み込み中です","読み終わったら自動で切り替わりますので、しばらくお待ちください。"),"#2C7A99");' +
        'me.wait=function(){exOvHide_();exWaitCtx(cb);};return;}' +
    ...
    'exEl("expkitems").addEventListener("click",function(e){var b=e.target.closest("[data-expk]");if(!b)return;' +
      'var i=parseInt(b.getAttribute("data-expk"),10);rvitems[i].do=!rvitems[i].do;exDrawPick();});' +
    'exEl("expknew").addEventListener("click",function(e){var b=e.target.closest("[data-expknew]");if(!b)return;' +
      'rvNewItems.splice(parseInt(b.getAttribute("data-expknew"),10),1);exDrawPick();});' +
    'exEl("expkAdd").onclick=function(){EXMENUBACK="exPickItems";showRvMenu("");};' +
    'exEl("expkGo").onclick=function(){if(!exChosen().length){szPopup_("今回やる施術を1つ以上選んでください。");return;}exShowAdd();};' +
    'exEl("exbackAdd").onclick=function(){exShowAsk();};' +
    'exEl("exaddlist").addEventListener("click",function(e){var b=e.target.closest("[data-exaddel]");if(!b)return;' +
      'rvNewItems.splice(parseInt(b.getAttribute("data-exaddel"),10),1);exShowAdd();});' +
    'exEl("exaddyes").onclick=function(){EXMENUBACK="exAdd";showRvMenu("");};' +
    ...
    'function exHideA_(){var ids=["exrvprev","exrvitemsh","exrvsuggest","exrvnew","exrvAddBtn","exrvsingle"];' +
      ...
      'var ch=exEl("exrvcnth");if(ch)ch.style.display="none";}' +
    ...
    'function exShowConfirm(){exOpen("exResv");exHideA_();EXSTEP="confirm";' +
      ...
      'var _h2=exEl("exrvhead");if(_h2)_h2.style.display="none";' +
      /* ★2026-09-25 まるちゃん「回数なおすところいらない・確認の見出しも削除」 */
      'var _ch2=exEl("exrvcnth");if(_ch2)_ch2.style.display="none";' +
      'var _cb=exEl("exrvcntbox");if(_cb)_cb.style.display="none";' +
    ...
    'function doRegisterMulti(){var decisions=[];for(var i=0;i<rvitems.length;i++){var it=rvitems[i];decisions.push({line_no:it.line_no,do:it.do,count:it.count,finish:it.finish});}var effM=exSlotKeys();var slots=[];for(var m=0;m<effM.length;m++){var mk=effM[m];var cf=rvSlotCfg_(mk);var rk=(cf.room==="STAR/福/🇫🇷")?"STAR":cf.room;slots.push({mark:exSlotMark(mk),room_key:rk,staff_emoji:(SEMO_[cf.staff]||""),dur:parseInt(cf.dur,10)||30});}rvLastSlotCount=slots.length;var fields={number:disp(),date:rvctx._ymd,time:rvctx._tm,prev_title:rvctx.prev_title||"",staff_emoji:SEMO_[rvsel.staff]||"",suffix:rvSuffix(),new_memo:rvctx.new_memo||"",decisions:decisions,new_items:rvNewItems.slice(),booking_services:rvBookingTexts(),paid:exPaid(),memo_override:(rvMemoOv!=null?rvMemoOv:""),slots:slots};exOvShow_(szBusyHtml_(slots.length+"枠の予約を登録中です"),"#2C7A99");jsonp({action:"submit",key:KEY,op:"existing_create_multi",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify(fields)},function(r){if(!r||!r.ok||!r.id){exOvHide_();szPopup_("エラーが発生しました。通信に失敗しました。もう一度お試しください。");return;}pollRegisterMulti(r.id);});}' +
    ...
    'function hideSteps(){var ids=["exNum","exPick","exMenu","exDate","exTime","exEdit","exMemo","exStaffPick","exRoomPick","exResv","exrvMenu","exAsk","exPickItems","exAdd","exSplit","exGroup","exDur","sgbackbar","sgMonth","sgDay","sgFree","sgTime"];for(var i=0;i<ids.length;i++){var el=document.getElementById(ids[i]);if(el)el.style.display="none";}setExTop();}' +
    ...
    /* ★2026-09-17 まるちゃん決定：番号のあとは、施術後の予約と同じ部品（月→日→その日の空き状況→予約時間設定）。
       部品の中の名前（drawCal・goTime など）がこの画面の「変更」用の名前とぶつからないよう、別の箱（SGX）に入れる。 */
    ...
    'function exPrefetch(date){if(PREF&&PREF.date===date&&!PREF.err)return;var me={date:date,ctx:null,err:"",wait:null};PREF=me;' +
      'jsonp({action:"submit",key:KEY,op:"existing_build_ctx",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify({number:disp(),date:date,time:""})},function(r){' +
        ...
        'function tryData(){if(PREF!==me||me.ctx||me.err)return;n++;jsonp({action:"data",name:nm},function(ctx){' +
          'if(PREF!==me)return;if(ctx&&ctx.ok){me.ctx=ctx;exPrefDone(me);return;}' +
          'if(n%3===0){jsonp({action:"status",key:KEY,id:r.id},function(st){if(PREF!==me)return;' +
            'if(!st||!st.ok){setTimeout(tryData,700);return;}' +
            'if(st.status==="pending"||st.status==="running"||st.status==="queued"||st.status===""||st.status==="done"){setTimeout(tryData,st.status==="done"?0:700);return;}' +
            'me.err=esc(st.result)||"エラーが発生しました。りゅうさんにお伝えください。";exPrefDone(me);});return;}' +
          'setTimeout(tryData,700);});}' +
        'tryData();});}' +
    /* ★2026-09-25 まるちゃん決定：既存の予約（LINEで予約を受けた時）は「次回の費用は支払済みですか？」を聞かない
       （その場にお客様がいないのでスタッフが調べようがない）。代わりに事務所パソコンが帳簿を見て、
    ...
    'function exGoMemo(st){if(!(EXFLOW&&PREF&&PREF.ctx)&&(!PREF||PREF.date!==st.date))exPrefetch(st.date);var me=PREF;' +
      ...
      'if(!me.ctx.found){szPopup_("「"+disp()+"」の前回の予約が見つかりません。");return;}' +
      /* ★2026-09-25：既存の予約は支払いを聞かないので、ここで時間の画面へ戻す必要がなくなった（決まりは上のとおり）。 */
    ...
    'var SGX=(function(){' +
      ...
      'function show(){SWAP=Date.now();hideSteps();var m={3:"sgMonth",4:"sgDay",5:"sgFree",6:"sgTime"};' +
      ...
      'function back(){if(step===6){goFree();}else if(step===5){goDay();}else if(step===4){goMonth();}' +
        'else{step=0;hideSteps();$("exNum").style.display="";window.scrollTo(0,0);}}' +
      '$("sgback").onclick=function(){if(!tapOK())return;back();};' +
      'sgBindSteps();' +
    ...
    '})();' +
    '})();</script>';
  // ★共通の部品（施術後の予約と同じ画面）は .ex の外に置く＝この画面のボタンの見た目が混ざらない。
  return '<style>' + HOMECSS_ + css + (isChange ? '' : sgCss_() + AKFCSS_) + '</style>' +
...
function renderClaudeToolsPage_(base, staff, dev) {
  ...
  var KOTOBA = [
    ...
    { t: 'キャンペーン設定ツール', c: 'キャンペーン設定',
      d: '新しいキャンペーンを始める時に、各種LINK・公式LINEのあいさつの自動応答文・前日お知らせの文と画像への登録を1つずつ手伝う。終わった翌日の朝9時に、LINK・応答文・インスタのキャンペーン投稿を自動で片づける（失敗したら4時間ごとに3回まで、だめならLINEで知らせる）。延長の時も「キャンペーン設定」と言うだけ' }
  ];
  var CSS =
    '  .ubar { display:flex; align-items:center; gap:12px; margin:0 0 4px; }' +
    '  .uhome { flex:0 0 auto; font-size:.9rem; font-weight:700; color:var(--ink,#1c2430); text-decoration:none;' +
...
/**
 * ★LINE一斉配信「キャンペーン・商品の紹介」（2026-09-17 まるちゃん依頼）。
 *   ①各種LINKの「キャンペーン／商品」から種類を選ぶ →②日本男性・日本女性・台湾男性・台湾女性の4画面で、
 *   それぞれ「未来店」「来店済」の2つのタブに文章（自分で入れる）と画像（各種LINKの画像から選ぶ）を入れる
 *   →③いつもの最終確認で日時を決めて予約（送り先は事務所パソコンの型「キャンペーン・商品の紹介」）。
 *   ★この関数は呼ばない。中身の文だけを renderBroadcastPage_ の画面の仕掛けの中へ差し込む（bcCampSrc_）。
 *     画面の仕掛けと同じ入れ物（TPL・DATA・draw など）をそのまま使うため。
...
function bcCampCode_() {
  ...
  function campCell(gi) {
    ...
    if (!c.parts) {                       // 前の持ち方（文章1つ＋画像の並び）から引っ越す
      var ps = [];
      var im = c.imgsSet ? (c.imgs || []) : campDefImgs(gi);
      im.forEach(function (u) { ps.push({ k: 'img', url: u }); });
      ps.push({ k: 'txt', text: c.text || '' });
      c.parts = ps.slice(0, MAXP); c.set = !!c.imgsSet;
      delete c.text; delete c.imgs; delete c.imgsSet;
    }
    if (!c.set && CLINKS) {               // まだ触っていない＝画像はその相手に合う物を選んだ状態にそろえる
      var txts = c.parts.filter(function (x) { return x.k === 'txt'; });
      if (!txts.length) txts = [{ k: 'txt', text: '' }];
      c.parts = campDefImgs(gi).map(function (u) { return { k: 'img', url: u }; }).concat(txts).slice(0, MAXP);
    }
    return c;
  }
  function campText(c) {
    return ((c && c.parts) || []).filter(function (x) { return x.k === 'txt' && String(x.text || '').replace(/^\s+|\s+$/g, ''); });
  }
  function campFilled(c) { return campText(c).length > 0; }
  function campAny() { for (var k in CDATA) if (campFilled(CDATA[k])) return true; return false; }
  function campTopic() {
    var ts = (CLINKS && CLINKS.topics) || [];
    for (var i = 0; i < ts.length; i++) if (ts[i].name === CKIND) return ts[i];
    return null;
  }
  ...
  function campLoadLinks() {
    var base = location.origin + location.pathname.replace(/[^/]*$/, '');
    CLINKERR = '';
    fetch(base + 'links.json?cb=' + Date.now(), { cache: 'no-store' })
  ...
  function campEnter() {
    ...
    if (!CLINKS) campLoadLinks();
    if (!CTPL.length) campLoadTpl();
    draw();
  }
  // 最終確認のために入れ替えた送り先・中身を、予約可能枠案内の物へ戻す
  function campUnswap() {
    if (!CORIG) return;
    TPL = CORIG.tpl; DATA = CORIG.data; CAT = CORIG.cat; TAGS = CORIG.tags; CORIG = null;
  }
  function campLeave() { campUnswap(); CMODE = false; }

  // ── 途中の作業を覚える（予約可能枠案内とは別の置き場）──
  function campSave() {
    if (!WIPON) return;
    if (CWIPT) clearTimeout(CWIPT);
    CWIPT = setTimeout(function () {
      CWIPT = null;
      if (!campAny()) return;
      try { pushImage(CAMPWIP, { t: Date.now(), kind: CKIND, gi: CGI, tab: CTAB, page: page, data: CDATA }); } catch (e) {}
    }, 1200);
  }
  function campWipClear() {
    if (CWIPT) { clearTimeout(CWIPT); CWIPT = null; }
    try { pushImage(CAMPWIP, { t: Date.now(), data: {} }); } catch (e) {}
  }

  // ── ①種類をえらぶ ──
  function drawCampKind() {
    var h = '<div class="bcstop"><span class="bcsttl">キャンペーン・商品の紹介</span></div>';
    if (CLINKERR) {
      h += '<div class="bcstatus ng">' + esc(CLINKERR) + '</div>';
    } else if (!CLINKS) {
      h += '<div class="bccard"><div class="bcouttx">各種LINKを読んでいます…</div></div>';
    } else {
      var ts = CLINKS.topics || [];
      ['キャンペーン', '商品'].forEach(function (k) {
        var list = ts.filter(function (t) { return (t.kind === '商品' ? '商品' : 'キャンペーン') === k; });
        h += '<div class="bccamph">' + (k === '商品' ? '🛍️ 商品' : '🎁 キャンペーン') + '</div>';
        if (!list.length) { h += '<div class="bccard"><div class="bcempty">各種LINKに登録がありません。</div></div>'; return; }
        h += '<div class="bccampkinds">' + list.map(function (t) {
          return '<button type="button" class="bccampkind' + (t.name === CKIND ? ' on' : '') + '" data-ck="' + esc(t.name) + '">' +
            esc(t.name) + '</button>';
        }).join('') + '</div>';
      });
    }
    h += '<button type="button" class="bctxt" id="bccrest" style="margin-top:6px">作業中のデータを復元する</button>';
    box.innerHTML = h;
    [].slice.call(box.querySelectorAll('[data-ck]')).forEach(function (b) {
      b.onclick = function () {
        var nm = b.getAttribute('data-ck');
        function go() { if (nm !== CKIND) { CDATA = {}; } CKIND = nm; CGI = 0; CTAB = 0; page = 'ce'; status(''); draw(); }
        if (CKIND && nm !== CKIND && campAny()) {
          szPopup_('入れた中身を消して「' + nm + '」に切り替えますか？', { cancel: true, onYes: go });
          return;
        }
        go();
      };
    });
    var rs = document.getElementById('bccrest');
    if (rs) rs.onclick = function () {
      status('作業中のデータを探しています…');
      jsonp({ action: 'data', name: CAMPWIP }, function (d) {
        var s = (d && d.data && typeof d.data === 'object') ? d : null;
        var any = false;
        if (s) for (var k in s.data) if (campFilled(s.data[k])) any = true;
        if (!any || !s.kind) { status('作業中のデータはありません。', true); return; }
        CDATA = s.data; CKIND = s.kind;
        CGI = (s.gi >= 0 && s.gi < CGROUPS.length) ? s.gi : 0; CTAB = s.tab ? 1 : 0;
        page = 'ce'; status('作業中のデータを復元しました。'); draw();
      });
    };
  }
  ...
  function drawCampEdit() {
    ...
    c.parts.forEach(function (x, i) {
      ...
      h += '<div class="bcslot"><div class="bcslothead"><span class="bcpno">' + (i + 1) + 'つ目　' + kind + '</span>' +
        '<span class="bcpbtns">' +
        (i > 0 ? '<button type="button" class="bcmv" data-su="' + i + '">▲ 上へ</button>' : '') +
        (i < n - 1 ? '<button type="button" class="bcmv" data-sd="' + i + '">▼ 下へ</button>' : '') +
        '<button type="button" class="bcdel" data-sx="' + i + '">消す</button></span></div>';
      if (x.k === 'img') {
        if (!imgs.length) {
          h += '<div class="bcempty">この案内の' + esc(g[2]) + 'には、各種LINKに画像がありません。</div>';
        } else {
          h += '<div class="bcgrid">' + imgs.map(function (u, j) {
            var on = x.url === u;
            return '<div class="bcpick' + (on ? ' bccampon' : '') + '">' +
              '<button type="button" class="sel" data-sp="' + i + ':' + j + '"><img src="' + esc(u) + '">' +
              '<span>' + (on ? '✓ この画像を送る' : 'この画像にする') + '</span></button>' +
              '<button type="button" class="bczoom" data-cz="' + j + '">🔍 大きく見る</button></div>';
          }).join('') + '</div>';
        }
      } else if (x.k === 'txt') {
        ...
        h += '<div class="bcadd"><button type="button" data-sk="' + i + ':img">🖼 画像にする</button>' +
          '<button type="button" data-sk="' + i + ':txt">✍ 文章にする</button></div>';
      }
      h += '</div>';
    });
    ...
    [].slice.call(box.querySelectorAll('[data-st]')).forEach(function (ta) {
      ...
      function count() {
        // ★前後の空白は数えない（送る時に取る・LINEの管理画面も数えない）
        var L = trimLen(ta.value), over = L > MAXT;
        cnt.textContent = L ? (L + ' / ' + MAXT + ' 文字' + (over ? '（あと' + (L - MAXT) + '文字減らしてください）' : '')) : '';
        cnt.className = 'bccount' + (over ? ' over' : '');
      }
      function grow() { ta.style.height = '0px'; ta.style.height = (ta.scrollHeight + 12) + 'px'; }
      count(); grow(); setTimeout(grow, 0);
      // ★打っている間は画面を描き直さない（描き直すと欄が作り直されて打てなくなる）
      ta.oninput = function () { c.parts[i].text = ta.value; count(); grow(); markTab(); saveNow(); };
    });
    [].slice.call(box.querySelectorAll('[data-ct]')).forEach(function (b) {
      b.onclick = function () { CTAB = +b.getAttribute('data-ct'); status(''); saveNow(); draw(); };
    });
    [].slice.call(box.querySelectorAll('[data-su]')).forEach(function (b) {
      b.onclick = function () { var i = +b.getAttribute('data-su'), v = c.parts[i]; c.parts[i] = c.parts[i - 1]; c.parts[i - 1] = v; changed(); };
    });
    [].slice.call(box.querySelectorAll('[data-sd]')).forEach(function (b) {
      b.onclick = function () { var i = +b.getAttribute('data-sd'), v = c.parts[i]; c.parts[i] = c.parts[i + 1]; c.parts[i + 1] = v; changed(); };
    });
    [].slice.call(box.querySelectorAll('[data-sx]')).forEach(function (b) {
      b.onclick = function () {
        var i = +b.getAttribute('data-sx'), x = c.parts[i];
        function go() { c.parts.splice(i, 1); changed(); }
        if (x.k === 'txt' && trimLen(x.text)) { szPopup_(i + 1 + 'つ目の文章を消しますか？', { cancel: true, onYes: go }); return; }
        go();
      };
    });
    [].slice.call(box.querySelectorAll('[data-sk]')).forEach(function (b) {
      b.onclick = function () {
        var a = b.getAttribute('data-sk').split(':'), i = +a[0];
        c.parts[i] = (a[1] === 'img') ? { k: 'img', url: imgs[0] || '' } : { k: 'txt', text: '' };
        changed();
      };
    });
    [].slice.call(box.querySelectorAll('[data-sp]')).forEach(function (b) {
      b.onclick = function () { var a = b.getAttribute('data-sp').split(':'); c.parts[+a[0]].url = imgs[+a[1]]; changed(); };
    });
    [].slice.call(box.querySelectorAll('[data-cz]')).forEach(function (b) {
      b.onclick = function () { bigView(imgs[+b.getAttribute('data-cz')], ''); };
    });
    ...
    document.getElementById('bccnext').onclick = function () {
      ...
      saveNow();
      if (!last) { CGI++; CTAB = 0; status(''); draw(); return; }
      campToLast();
    };
  }
  ...
  function campToLast() {
    ...
    DATA = CTPL.map(function (t) {
      ...
      x.parts.forEach(function (sl) {
        if (sl.k === 'img' && sl.url) ps.push({ kind: 'image', src: 'link:' + sl.url, thumb: sl.url });
        var tx = (sl.k === 'txt') ? String(sl.text || '').replace(/^\s+|\s+$/g, '') : '';
  ...
  // 予約できたあと＝入れた中身と覚えていた途中の作業を消し、予約可能枠案内の物へ戻す
  function campDone() {
  ...
  function campBack() {
    if (page === 'c') { campLeave(); page = 'k'; return true; }
    if (page === 'ce') { if (CGI > 0) { CGI--; CTAB = 0; } else { page = 'c'; } return true; }
    if (LMODE === 'at') { LMODE = ''; return true; }
    campUnswap(); page = 'ce'; CGI = CGROUPS.length - 1; step = 0; return true;
  }
}
