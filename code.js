...
function renderExistingPage_(base, staff, dev, mode) {
  ...
  var script = '<script>(function(){' +
    ...
    'function doneMsg(){if(chgtype==="dur"){return "施術時間 "+esel.dur+"分 へ";}if(chgtype==="memo"){return "予約メモを";}if(chgtype==="staff"){return "担当 "+(SNM_[esel.staff]||"")+" へ";}if(chgtype==="room"){return "部屋 "+esel.room+" へ";}var lines=[];if(chgtype==="dt"){lines.push((picked.getMonth()+1)+"月"+picked.getDate()+"日 "+tdisp()+" へ");}else if(chgtype==="t"){lines.push("時刻 "+tdisp()+"（同じ日）へ");}if(pickStaff){lines.push("担当を"+(SEMO_[pickStaff]||"")+" "+(SNM_[pickStaff]||"")+"へ");}if(pickRoom){lines.push("部屋を"+roomBadge_(pickRoom,true)+"へ");}return lines.join("<br>");}' +
    'var TOPHREF="' + topHref + '";' +
