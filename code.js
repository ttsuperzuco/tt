...
function renderExistingPage_(base, staff, dev, mode) {
  ...
  var resvSec =
    '<div id="exResv" style="display:none">' +
      ...
      '<div class="exsec">タイトル（実際に付く名前）</div><div class="exrvtitle" id="exrvtitle"></div>' +
  ...
  var script = '<script>(function(){' +
    ...
    'function rvAutoEye(){var hw=(rvctx.hair_words||[]);var doing=rvDoingTexts().join(" ");for(var i=0;i<hw.length;i++){if(doing.indexOf(hw[i])>=0)return true;}return false;}' +
    ...
    'function rvDrawMarks(){var tm=(rvctx.title_marks||[]);var h="";for(var i=0;i<tm.length;i++){var mk=tm[i][0];h+="<button type=\\"button\\" class=\\"exp"+(rvMarkOn(mk)?" sel":"")+"\\" data-mark=\\""+esc(mk)+"\\" style=\\"background:#475569\\">"+esc(mk)+"</button>";}document.getElementById("exrvmarks").innerHTML=h;var sf=document.querySelectorAll("#exrvsuf .exp");for(var j=0;j<sf.length;j++){var k=sf[j].getAttribute("data-suf");sf[j].classList.toggle("sel",(k==="dochi")?rvDochi:rvEyeOn());}var tt=document.getElementById("exrvtitle");if(tt)tt.textContent=rvTitle();rvDrawSlots();}' +
    ...
    'document.getElementById("exrvmarks").addEventListener("click",function(e){var b=e.target.closest(".exp");if(!b)return;var mk=b.getAttribute("data-mark");rvMarkOv[mk]=!rvMarkOn(mk);rvDrawMarks();});' +
    'document.getElementById("exrvsuf").addEventListener("click",function(e){var b=e.target.closest(".exp");if(!b)return;var k=b.getAttribute("data-suf");if(k==="dochi"){rvDochi=!rvDochi;}else{rvEyeOv=!rvEyeOn();}rvDrawMarks();});' +
    ...
    'document.getElementById("exrvitems").addEventListener("click",function(e){var card=e.target.closest(".rvcard");if(!card)return;var i=parseInt(card.getAttribute("data-i"),10);var it=rvitems[i];var act=e.target.getAttribute("data-act");if(!act)return;if(act==="do"){it.do=true;it.finish=false;}else if(act==="skip"){it.do=false;it.finish=false;}else if(act==="fin"){it.finish=true;it.do=false;}else if(act==="dec"){it.count=(it.count==null?1:Math.max(1,it.count-1));}else if(act==="inc"){it.count=(it.count==null?1:it.count+1);}drawRvItems();rvDrawMarks();});' +
