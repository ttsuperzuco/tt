...
function renderExistingPage_(base, staff, dev, mode) {
  ...
  var script = '<script>(function(){' +
    ...
    'var avail=null;' +
    'function loadAvail(){avail=null;jsonp({action:"submit",key:KEY,op:"availability",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify({date:chosen.date,start_min:(chosen.start_min!=null?chosen.start_min:0),dur:(chosen.dur_min||30),cur_room:chosen.room,cur_staff:(chosen.staff_emoji||"")})},function(r){if(!r||!r.ok||!r.id){return;}pollAvail(r.id);});}' +
    ...
    'function doneMsg(){var sn={"1":"トマト","2":"みかん","3":"オリーブ","4":"マンゴー"};if(chgtype==="dt"){return (picked.getMonth()+1)+"月"+picked.getDate()+"日 "+tdisp()+" へ";}if(chgtype==="t"){return "時刻 "+tdisp()+"（同じ日）へ";}if(chgtype==="dur"){return "施術時間 "+esel.dur+"分 へ";}if(chgtype==="memo"){return "予約メモを";}if(chgtype==="staff"){return "担当 "+(sn[esel.staff]||"")+" へ";}if(chgtype==="room"){return "部屋 "+esel.room+" へ";}return "";}' +
    'var TOPHREF="' + topHref + '";' +
