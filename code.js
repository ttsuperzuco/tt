...
function renderExistingPage_(base, staff, dev, mode) {
  ...
  var css =
    ...
    '.expickrow .pm{color:#475569;font-weight:700;font-size:25px;display:block;margin-top:6px;line-height:1.4;}' +
  ...
  var script = '<script>(function(){' +
    ...
    'function loadPicks(){document.getElementById("exNum").style.display="none";document.getElementById("exPick").style.display="";document.getElementById("expickwho").textContent="「"+disp()+"」の予約";document.getElementById("expicklist").innerHTML="";document.getElementById("exPickGo").style.display="none";document.getElementById("expickst").textContent="予約をさがしています…（10秒ほどかかります）";window.scrollTo(0,0);jsonp({action:"submit",key:KEY,op:"customer_reservations",who:idn.who,role:idn.role,device:idn.device,fields:JSON.stringify({number:disp()})},function(r){if(!r||!r.ok||!r.id){document.getElementById("expickst").textContent="依頼を送れませんでした："+((r&&r.error)||"不明");return;}setTimeout(function(){pollPicks(r.id);},500);});}' +
    'function renderPicks(list){var el=document.getElementById("expicklist");document.getElementById("expickst").textContent=list.length?"":"この番号の予約が見つかりません。";var h="";for(var i=0;i<list.length;i++){var r=list[i];var pm=(r.parts||[]).join("・")||"—";var past=r.is_past?"（過去）":"";var nm=esc(r.name||"");h+="<button class=\\"expickrow\\" data-i=\\""+i+"\\">"+(nm?"<span class=\\"pn\\">"+nm+" 様</span>":"")+"<span class=\\"pd\\">"+past+r.date+" "+r.start_hm+" "+(r.dur_min||"")+"分</span><span class=\\"pm\\">部屋 "+esc(r.room)+"　担当 "+esc(r.staff_emoji||"?")+"　"+esc(pm)+"</span></button>";}el.innerHTML=h;window.__PICKS=list;var rows=el.querySelectorAll(".expickrow");for(var j=0;j<rows.length;j++){(function(k){rows[k].addEventListener("click",function(){for(var m=0;m<rows.length;m++){rows[m].classList.remove("sel");}rows[k].classList.add("sel");chosen=window.__PICKS[k];document.getElementById("exPickGo").style.display="";});})(j);}}' +
